...
from mcp_pal_cli import supervisor
...
def test_ui_server_prints_links_and_returns_original_failure(monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]) -> None:
    class Child:
    ...
    monkeypatch.setattr(supervisor.time, "sleep", lambda _seconds: (_ for _ in ()).throw(KeyboardInterrupt()))
...
def test_ui_server_zero_runs_prints_message(monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]) -> None:
    class Child:
        process = SimpleNamespace(poll=lambda: None)
...
def test_ui_server_readiness_failure_is_cleaned_up(monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]) -> None:
    class Child:
        process = SimpleNamespace(poll=lambda: None)
        lines = deque(["API_KEY=top-secret", "safe startup failure"])
    ...
    monkeypatch.setattr(supervisor, "_terminate_process", lambda process: stopped.append(process))
...
def test_find_new_runs_excludes_existing_and_sorts_stably() -> None:
    ...
    after = supervisor.StoredRuns((_run("z", 3), _run("b", 2), _run("a", 2), _run("existing", 4), _run("b", 2)))
    assert supervisor.find_new_runs(before, after) == (_run("a", 2), _run("b", 2), _run("z", 3))


def test_list_stored_runs_reads_all_pages_and_uses_visible_store_results(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    class Page:
    ...
    class Store:
        ...
        def list_executions(self, *, limit: int, offset: int) -> Page:
            ...
            if offset == 0:
                return Page(tuple(Snapshot(f"run-{index}", index) for index in range(100)), 102)
    ...
    monkeypatch.setattr(supervisor, "_execution_store_type", lambda: lambda _database: store)
...
def test_list_stored_runs_returns_safe_warning_for_unreadable_database(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    secret_path = tmp_path / "private-token.sqlite"
    monkeypatch.setattr(supervisor, "_execution_store_type", lambda: (_ for _ in ()).throw(RuntimeError("secret")))
...
def test_run_urls_encode_every_path_separator_character() -> None:
    ...
    assert urls.direct == "http://127.0.0.1:8123/playground/run/run%20id%2Fwith%3Funsafe%23chars"
...
def test_cli_forwards_exact_args_after_separator(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(supervisor, "validate_project_python", lambda *_args, **_kwargs: "0.2.0a2")
...
def test_posix_termination_handlers_are_scoped(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(supervisor.signal, "signal", lambda signum, handler: installed.append((int(signum), handler)))
...
def test_sigterm_child_cleanup_returns_signal_status(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(supervisor, "_terminate_process", lambda child: terminated.append(child))
    assert supervisor.run_test(project_root=tmp_path) == 128 + int(signal.SIGTERM)
    assert terminated == [process]


def test_keyboard_interrupt_cleans_child_and_returns_interrupt_status(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
...
def test_resolution_prefers_explicit_then_virtualenv_then_conda_then_project_venv(
    ...
    local.touch()
    monkeypatch.setattr(supervisor.shutil, "which", lambda name, **_: "/path/python" if name == "python3" else None)

    assert supervisor.resolve_project_python(explicit, project_root=tmp_path) == explicit.resolve()
    assert supervisor.resolve_project_python(project_root=tmp_path, environment={"VIRTUAL_ENV": str(virtual.parent.parent), "CONDA_PREFIX": str(conda.parent.parent)}) == virtual.resolve()
    virtual.unlink()
    assert supervisor.resolve_project_python(project_root=tmp_path, environment={"VIRTUAL_ENV": str(virtual.parent.parent), "CONDA_PREFIX": str(conda.parent.parent)}) == conda.resolve()
    conda.unlink()
    assert supervisor.resolve_project_python(project_root=tmp_path, environment={"VIRTUAL_ENV": str(virtual.parent.parent), "CONDA_PREFIX": str(conda.parent.parent)}) == local.resolve()


def test_resolution_uses_python3_then_python(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    seen: list[str] = []
    ...
    assert supervisor.resolve_project_python(project_root=tmp_path, environment={}) == Path("/tool/python").resolve()
    assert seen == ["python3", "python"]


def test_resolution_uses_passed_path_not_ambient_path(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(supervisor.shutil, "which", which)
    assert supervisor.resolve_project_python(project_root=tmp_path, environment={"PATH": "/passed/bin"}) == Path("/passed/python").absolute()
    assert seen == [("python3", "/passed/bin")]


def test_windows_candidate_layouts(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    local.parent.mkdir(parents=True)
    local.touch()
    assert supervisor.resolve_project_python(project_root=tmp_path, environment={"VIRTUAL_ENV": str(virtual.parents[1])}) == virtual.absolute()
    virtual.unlink()
    assert supervisor.resolve_project_python(project_root=tmp_path, environment={"CONDA_PREFIX": str(conda.parent)}) == conda.absolute()
    conda.unlink()
    assert supervisor.resolve_project_python(project_root=tmp_path, environment={}) == local.absolute()


def test_rejected_candidate_does_not_fall_back(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    candidate = tmp_path / "python"
    candidate.touch()
    monkeypatch.setattr(supervisor, "validate_project_python", lambda *_args, **_kwargs: (_ for _ in ()).throw(supervisor.ProjectPythonError("missing required MCP Pal packages")))
    assert supervisor.run_test(python=candidate, project_root=tmp_path) == 2


def test_validation_requires_exact_sdk_version(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    result = type("Result", (), {"returncode": 0, "stdout": json.dumps({"checks": {"pytest": True, "mcp_pal": True, "mcp_pal.pytest_plugin": True, "SQLiteExecutionStore": True}, "version": "0.2.0a1"})})()
    ...
    with pytest.raises(supervisor.ProjectPythonError, match="does not match"):
        supervisor.validate_project_python(Path(sys.executable), cli_sdk_version="0.2.0a2", project_root=tmp_path)


def test_command_uses_selected_python_and_absolute_database(tmp_path: Path) -> None:
    command = supervisor.pytest_command(Path("/project/.venv/bin/python"), (tmp_path / "results.sqlite").resolve(), ["-q", "tests"])
...
def test_real_subprocess_runs_plugin_and_keeps_pytest_summary(tmp_path: Path, capfd: pytest.CaptureFixture[str]) -> None:
...
def test_plain_test_does_not_scan_results_database(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    process = _Process(0)
    monkeypatch.setattr(supervisor, "list_stored_runs", lambda *_args: pytest.fail("plain test scanned history"))
    monkeypatch.setattr(supervisor, "resolve_project_python", lambda *_args, **_kwargs: Path(sys.executable))
    monkeypatch.setattr(supervisor, "validate_project_python", lambda *_args, **_kwargs: "0.2.0a2")
    monkeypatch.setattr(supervisor.subprocess, "Popen", lambda *_args, **_kwargs: process)
    assert supervisor.run_test(project_root=tmp_path) == 0
...
def test_real_store_listing_excludes_deleted_runs(tmp_path: Path) -> None:
    ...
    assert [run.run_id for run in supervisor.list_stored_runs(database).runs] == ["visible"]
...
def test_pytest_exit_code_is_forwarded(monkeypatch: pytest.MonkeyPatch, tmp_path: Path, code: int) -> None:
    process = type("Process", (), {"pid": 1, "wait": lambda self, **_: code, "poll": lambda self: 0})()
    monkeypatch.setattr(supervisor, "resolve_project_python", lambda *_args, **_kwargs: Path(sys.executable))
    monkeypatch.setattr(supervisor, "validate_project_python", lambda *_args, **_kwargs: "0.2.0a2")
    monkeypatch.setattr(supervisor.subprocess, "Popen", lambda *_args, **_kwargs: process)
