"""Project-local MCP Pal SDK setup.
...
def _download_release(version: str, directory: Path) -> Path:
    sdk_name = f"mcp_pal-{version}-py3-none-any.whl"
    cli_name = f"mcp_pal_cli-{version}-py3-none-any.whl"
    app_name = f"mcp_pal_app-{version}-py3-none-any.whl"
    ...
    try:
        ...
        raise SetupError("MCP Pal SDK release is unavailable") from None
    if actual != expected:
        raise SetupError("MCP Pal SDK checksum verification failed")
    return sdk
...
def run(args: Any) -> int:
    ...
    print(f"Installing MCP Pal SDK {version}")
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="mcp-pal-setup-") as temporary:
        ...
        if not _ready(target.python, version, root):
            raise SetupError("project environment did not pass the MCP Pal SDK checks")
    except SetupError:
