...
def _usage(raw: dict[str, Any]) -> dict[str, Any]:
    ...
    if isinstance(stream, dict):
        candidates.extend([stream.get("usage"), (stream.get("message") or {}).get("usage") if isinstance(stream.get("message"), dict) else None])
    message = raw.get("message")
...
def _protocol_spans(protocol: list[dict[str, Any]], transport: str, base_id: str = "mcp") -> list[dict[str, Any]]:
    ...
    for record in sorted(protocol, key=lambda item: _float(item.get("offset_ms", 0))):
        ...
        if direction in {"client_to_server", "request"} and request_id is not None:
            pending[str(request_id)] = {"start": start, "method": method, "seq": seq, "record": record}
            continue
        ...
        if parent:
            ...
            span = {
                "id": f"{base_id}-{parent['seq']}", "parent_id": "run-mcp",
                "kind": "mcp", "name": name, "status": "error" if isinstance(payload, dict) and payload.get("error") else "completed",
                "start_ms": span_start, "end_ms": end, "duration_ms": _duration(span_start, end),
                "transport": transport, "input": parent["record"].get("payload"), "output": payload,
                "metadata": {"jsonrpc_id": request_id, "request_sequence": parent["seq"], "response_sequence": seq},
            }
            spans.append(span)
        else:
            spans.append({
                ...
                "output": payload if direction not in {"client_to_server", "request"} else None,
                "metadata": {"direction": direction, "jsonrpc_id": request_id, "capture_kind": record.get("kind", "jsonrpc")},
            })
    return spans
...
def build_claude_trace(*, events: Iterable[dict[str, Any] | str], protocol_events: Iterable[dict[str, Any]] = (), transport: str = "stdio", status: str = "completed", cost_usd: float | None = None, session_id: str | None = None) -> dict[str, Any]:
    ...
    for sequence, record in enumerate(protocol_events, 1):
        ...
        if isinstance(payload, dict):
            ...
            item.setdefault("status", "error" if payload.get("error") is not None else ("request" if payload.get("method") else "response"))
        item.setdefault("sequence", sequence)
        protocol_list.append(item)
    spans: list[dict[str, Any]] = [
        {"id": "run", "parent_id": None, "kind": "run", "name": "Claude run", "status": status, "start_ms": 0.0, "end_ms": None, "duration_ms": None, "transport": transport, "input": None, "output": None, "metadata": {"harness": "claude-code", "session_id": session_id}},
        {"id": "run-mcp", "parent_id": "run", "kind": "mcp_session", "name": f"MCP session · {transport}", "status": "completed" if protocol_list else "unobserved", "start_ms": 0.0, "end_ms": None, "duration_ms": None, "transport": transport, "input": None, "output": None, "metadata": {"transport": transport}},
    ]
    ...
    def message_id(raw: dict[str, Any], typ: str) -> str | None:
        if typ == "stream_event":
            ...
            message = stream.get("message") if isinstance(stream.get("message"), dict) else {}
    ...
    def ensure_turn(start: float, raw: dict[str, Any], typ: str, partial: bool) -> dict[str, Any]:
        ...
        if ident and ident in turns_by_message:
            ...
            if partial:
                pending_turn = turn
            return turn
        if not ident and not partial and pending_turn and pending_turn.get("partial") and not pending_turn.get("closed"):
            pending_turn["closed"] = True
            return pending_turn
        turn = {"id": f"turn-{len(turns) + 1}", "message_id": ident, "start": start, "partial": partial, "closed": False, "usage": {}}
        turns.append(turn)
        if ident:
            turns_by_message[ident] = turn
        pending_turn = turn
        spans.append({"id": turn["id"], "parent_id": "run", "kind": "model_turn", "name": f"Model turn {len(turns)}", "status": "streaming" if partial else "completed", "start_ms": start, "end_ms": start, "duration_ms": 0.0, "transport": transport, "input": None, "output": None, "metadata": {"usage": {}, "message_id": ident}, "tokens": {}})
        return turn

    def add_block(turn: dict[str, Any], block: dict[str, Any], start: float, block_index: Any, partial: bool) -> dict[str, Any] | None:
        ...
        if block_type in {"text", "text_delta"}:
            ...
            output = None if block_type == "signature_delta" else (block.get("thinking") or "")
            ...
            kind, name, output = "tool_call", "Tool input", block.get("partial_json", "")
        else:
            return None
        ...
        if kind == "tool_call":
            ...
            if ident:
                tool_calls[str(ident)] = {"span": span, "start": _float(span.get("start_ms")), "name": name, "turn": turn["id"]}
    ...
    for index, event in enumerate(ordered):
        ...
        if typ in {"assistant", "message"}:
            ...
            turn_span["metadata"]["usage"], turn_span["tokens"] = turn["usage"], turn["usage"]
            message = raw.get("message") if isinstance(raw.get("message"), dict) else {}
            ...
            if stream_type == "message_start":
                ...
                if delta.get("stop_reason"):
                    next(span for span in spans if span["id"] == pending_turn["id"])["metadata"]["stop_reason"] = delta["stop_reason"]
            ...
            for block_index, block in enumerate(blocks):
                ...
                if call:
                    span = call["span"]
                    span["kind"], span["name"] = "tool_call", f"MCP tool · {call['name']}"
                    span["status"], span["end_ms"] = ("error" if block.get("is_error") else "completed"), start
                    span["duration_ms"], span["output"] = _duration(call["start"], start), block.get("content", block)
                    span.setdefault("metadata", {})["correlation"] = "exact"
                else:
                    spans.append({"id": f"tool-result-{index}-{block_index}", "parent_id": "run", "kind": "tool_result", "name": "MCP tool result", "status": "error" if block.get("is_error") else "completed", "start_ms": start, "end_ms": start, "duration_ms": 0.0, "transport": transport, "input": None, "output": block.get("content", block), "metadata": {"tool_use_id": ident, "correlation": "unmatched"}})
        elif typ in {"result", "final"}:
            result_metadata = {key: raw[key] for key in ("duration_ms", "duration_api_ms", "num_turns", "total_cost_usd", "usage", "is_error", "subtype") if key in raw}
            spans.append({"id": "result", "parent_id": "run", "kind": "result", "name": "Final result", "status": "error" if raw.get("is_error") else status, "start_ms": start, "end_ms": start, "duration_ms": 0.0, "transport": transport, "input": None, "output": raw.get("result", raw.get("text")), "metadata": result_metadata})
            final_offset = max(final_offset, start)

    final_usage = _usage(raw_of(ordered[-1])) if ordered and isinstance(ordered[-1], dict) else {}
    if not any(turn["usage"] for turn in turns) and final_usage:
        if turns:
            ...
            spans.append({"id": "turn-1", "parent_id": "run", "kind": "model_turn", "name": "Model turn 1", "status": "completed", "start_ms": 0.0, "end_ms": final_offset, "duration_ms": final_offset, "transport": transport, "input": None, "output": None, "metadata": {"usage": final_usage}, "tokens": final_usage})
    ...
    end_ms = max(final_offset, max((_float(span.get("end_ms")) for span in spans), default=0.0))
