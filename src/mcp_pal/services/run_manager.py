...
from ..harness.base import RunSpec, AcpRunSpec
from ..harness.acp import AcpHarnessRunner
...
from ..persistence.models import McpProfileRevision, HarnessProfileRevision, RunHarnessSnapshot, Run, RunEvent, RunTrace, now
...
from ..trace.acp import build_acp_trace
...
class RunManager:
    ...
    def execute(self, run_id):
        ...
        try:
            ...
            safe_error, _ = redact(result.error)
            run.status,run.claude_result,run.exit_code,run.error_message,run.stderr=result.status,safe_result,result.exit_code,(safe_error if run.harness == "acp" else result.error),safe_stderr; run.cost_usd,run.turns,run.session_id=result.cost_usd,result.turns,result.session_id; run.mcp_assertion=derive_mcp_assertion(result.events,run.enabled_server); run.finished_at=now()
            if run.harness == "acp" and (result.error or result.error_code or result.error_details):
                ...
                if snapshot:
                    safe_details, _ = redact(result.error_details or {})
            ...
            if run.harness == "acp":
                ...
                if snapshot:
                    ...
                    safe_observed, _ = redact(observed)
                    verification=dict(snapshot.verification or {})
            ...
            if run.harness == "acp": exception_message=redact(exception_message)[0]
