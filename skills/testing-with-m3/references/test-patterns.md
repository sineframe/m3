...
Install project test support with `uv add "m3[pytest]"`. This installs the
...
`m3[pytest,storage]` and pass a SQLite store:
...
reopened = SQLiteExecutionStore(".m3/executions.sqlite")
view = reopened.get_trace_view(execution_id)
reopened.close()
```

`m3 test` selects SQLite automatically by loading the pytest plugin with
`--results-db`; direct pytest can opt into the same default-store
behavior:
...
`--results-db PATH` selection is required; otherwise SDK storage is in
memory. With the M3 pytest plugin, pytest item outcomes are saved in
internal run records and M3 matcher checks as execution evaluations.
