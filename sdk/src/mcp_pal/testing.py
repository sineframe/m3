...
import sys
import threading
from typing import Any as _Any, cast as _cast, overload as _overload, ParamSpec as _ParamSpec, TypeAlias as _TypeAlias, TypeVar as _TypeVar
...
from .types import InProcessServer as _InProcessServer, SSEServer as _SSEServer, StdioServer as _StdioServer, TrustLevel as _TrustLevel


_JsonValue: _TypeAlias = None | bool | int | float | str | list["_JsonValue"] | dict[str, "_JsonValue"]
...
class _FaultState:
    ...
    def observe_request(self, message: _Any) -> None:
        ...
        if isinstance(message, _types.JSONRPCNotification) and message.method == "notifications/cancelled":
            params = message.params or {}
            request_id = _request_id_key(params.get("requestId")) if isinstance(params, _Mapping) else None
            ...
            if (
                ...
                return _SessionMessage(
                    message=_types.JSONRPCNotification(
                        ...
                        params={"requestId": request_id[1] if request_id is not None else None},
                    )
                )
        return item
...
class _FaultWriteStream:
    ...
    async def _send_response(self, item: _SessionMessage, method: str | None) -> None:
        ...
        if not is_response or method is None:
            await self._send_inner(item)
            return
        if method in self._state.faults.malformed_methods or method in self._state.faults.partial_methods:
        ...
        if race_gate is not None:
            request_id = _request_id_key(getattr(message, "id", None))
            cancelled = self._state.cancellations.get(request_id) if request_id is not None else None
        ...
        if method in self._state.faults.invalid_result_methods and isinstance(message, _types.JSONRPCResponse):
...
class _FaultingServer:
    ...
    async def run(self, read_stream: _Any, write_stream: _Any, options: _Any, *, raise_exceptions: bool = False) -> None:
...
class RecordedArtifact:
    ...
    @classmethod
    def from_bytes(cls, artifact_id: str, content: bytes, *, media_type: str | None = None) -> "RecordedArtifact":
        if not isinstance(artifact_id, str) or not artifact_id:
            raise ValueError("artifact id must be a non-empty string")
        if not isinstance(content, bytes):
            raise TypeError("artifact content must be bytes")
        return cls(artifact_id, _hashlib.sha256(content).hexdigest(), len(content), media_type)
...
class _RedactionRuntime:
    ...
    __slots__ = ("config", "bound")

    def __init__(self, config: _RedactionConfig | None = None, *, bound: bool = False) -> None:
...
def _json_safe(value: _Any, *, _active: set[int] | None = None, _depth: int = 0) -> _JsonValue:
    ...
    try:
        ...
        if isinstance(value, (list, tuple)):
            ...
            return [_json_safe(item, _active=active, _depth=_depth + 1) for item in value]
        try:
            model_dump = getattr(value, "model_dump")
            if not callable(model_dump):
                raise TypeError("mock values must be JSON-compatible or Pydantic models")
...
class RecordedInteraction:
    ...
    def model(self) -> dict[str, _JsonValue]:
        value: dict[str, _JsonValue] = {
            "method": self.method, "params": dict(self.params), "sequence": self.sequence,
...
class Recording:
    ...
    redaction_bindings: tuple[RedactionBinding, ...] = _field(default_factory=lambda: (RedactionBinding(source="legacy-unverified"),))
    _runtime: _RedactionRuntime = _field(default_factory=_RedactionRuntime, repr=False, compare=False)
    ...
    def with_redaction_config(self, config: _RedactionConfig) -> "Recording":
        ...
        safe = type(self).from_json(json.dumps(projected, sort_keys=True, separators=(",", ":")))
    ...
    def validate_artifacts(self, contents: _Mapping[str, bytes]) -> None:
        ...
        if len(expected) != len(self.artifacts) or set(contents) != set(expected):
            raise ArtifactIntegrityError("recorded artifact set does not match supplied contents")
        for artifact_id, artifact in expected.items():
            try:
                artifact.validate(contents[artifact_id])
            except (ArtifactIntegrityError, KeyError):
                raise ArtifactIntegrityError("recorded artifact integrity check failed") from None

    def model_dump(self, *, config: _RedactionConfig | None = None) -> dict[str, _JsonValue]:
        if config is None and self.redaction_bound and not self._runtime.bound:
            raise ValueError("recording requires an explicit redaction config before serialization")
        raw: dict[str, _JsonValue] = {
            "schema": "mcp_pal.mock_recording.v1", "redaction_bound": self.redaction_bound,
            "provenance": list(self.provenance),
            ...
            "redaction_bindings": [binding.model() for binding in self.redaction_bindings],
        }
        projected = _redact_for_persistence(raw, config=config or self._runtime.config, path="$.recording")
    ...
    def to_json(self, *, config: _RedactionConfig | None = None) -> str:
        return json.dumps(self.model_dump(config=config), sort_keys=True, separators=(",", ":"))
    ...
    def redacted(self, *, config: _RedactionConfig | None = None) -> "Recording":
        ...
        projected = self.model_dump(config=config)
        return type(self).from_json(json.dumps(projected, sort_keys=True, separators=(",", ":")))
    ...
    def from_json(cls, value: str) -> "Recording":
        ...
        if not isinstance(document, _Mapping) or document.get("schema") != "mcp_pal.mock_recording.v1":
            raise ValueError("recording schema is unsupported")
        if set(document) - {"schema", "redaction_bound", "provenance", "server", "initialization", "artifacts", "interactions", "redaction_bindings"}:
        ...
        if not isinstance(raw_provenance, list) or not all(isinstance(part, str) for part in raw_provenance):
            raise ValueError("recording provenance is invalid")
        raw_server = document.get("server", {"name": "mock-mcp", "version": "1"})
        if not isinstance(raw_server, _Mapping) or set(raw_server) - {"name", "version"}:
        ...
        if not isinstance(server_name, str) or not isinstance(server_version, str) or not server_name or not server_version:
            raise ValueError("recording server identity is invalid")
        ...
        for raw_binding in raw_bindings:
            if not isinstance(raw_binding, _Mapping) or set(raw_binding) - {"source", "version", "wildcard_paths", "references"}:
                raise ValueError("recording redaction binding is invalid")
            ...
            if (
                ...
                or not isinstance(wildcard_paths, list) or not all(isinstance(item, str) for item in wildcard_paths)
                or not isinstance(secret_references, list) or not all(isinstance(item, str) for item in secret_references)
            ):
                raise ValueError("recording redaction binding is invalid")
            bindings.append(RedactionBinding(source, version, tuple(wildcard_paths), tuple(secret_references)))
        ...
        for raw_artifact in raw_artifacts:
            if not isinstance(raw_artifact, _Mapping) or set(raw_artifact) - {"artifact_id", "sha256", "size_bytes", "media_type"}:
            ...
            artifacts.append(RecordedArtifact(artifact_id, sha256, size_bytes, media_type))
        ...
        for item in raw_items:
            ...
            if set(item) - {"method", "params", "response", "error", "sequence", "provenance"}:
                raise ValueError("recording interaction contains unknown fields")
            if not isinstance(item.get("method"), str) or not isinstance(item.get("params"), _Mapping):
                raise ValueError("recording interaction is invalid")
            ...
            if isinstance(raw_sequence, bool) or not isinstance(raw_sequence, int) or raw_sequence != len(interactions):
            ...
            if not isinstance(interaction_provenance, list) or not all(isinstance(part, str) for part in interaction_provenance):
                raise ValueError("recording interaction provenance is invalid")
            interactions.append(RecordedInteraction(
                method=item["method"], params=_cast(_Mapping[str, _JsonValue], params),
                response=response,
        ...
        return cls(
            interactions=tuple(interactions), redaction_bound=document["redaction_bound"],
            provenance=tuple(raw_provenance), server_name=server_name, server_version=server_version,
            initialization=_cast(_Mapping[str, _JsonValue], initialization), artifacts=tuple(artifacts),
...
class FaultInjector:
    ...
    _http_close: _Callable[[], None] | None = _field(default=None, init=False, repr=False)
    ...
    def protocol_error(self, method: str, code: int = -32000, message: str = "mock protocol error") -> "FaultInjector":
    ...
    def stdio_server(self, *, name: str = "mcp-pal-wire-fault") -> _StdioServer:
        ...
        environment = {
            "MCP_PAL_WIRE_FAULTS": json.dumps(configuration, sort_keys=True, separators=(",", ":")),
            "PYTHONPATH": source_root,
        }
        return _StdioServer(
            name=name,
            command=sys.executable,
            args=("-m", "mcp_pal.testing_wire"),
            environment=environment,
            cwd=source_root,
        )

    def sse_server(self, *, name: str = "mcp-pal-sse-fault") -> _SSEServer:
    ...
    def response(self, method: str, value: _Any) -> _Any:
        ...
        if method in self.oversized_methods:
            ...
            if isinstance(value, _types.CallToolResult):
                value = value.model_copy(update={"content": [*value.content, _types.TextContent(text="x" * size)]})
            elif isinstance(value, _types.ReadResourceResult):
                value = value.model_copy(update={"contents": [*value.contents, _types.TextResourceContents(uri="fault://oversized", mime_type="text/plain", text="x" * size)]})
        return value
...
class MockMCPServer:
    ...
    def __init__(self, name: str = "mock-mcp", *, version: str = "1", strict: bool = True, redaction_config: _RedactionConfig | None = None, faults: FaultInjector | None = None, state: _Mapping[str, _Any] | None = None) -> None:
    ...
    @_overload
    def tool(self, name: str | None = None, *, description: str | None = None, input_schema: _Mapping[str, _Any] | None = None, output_schema: _Mapping[str, _Any] | None = None) -> _Callable[[_Callable[_P, _R]], _Callable[_P, _R]]: ...

    @_overload
    def tool(self, name: _Callable[_P, _R], *, description: str | None = None, input_schema: _Mapping[str, _Any] | None = None, output_schema: _Mapping[str, _Any] | None = None) -> _Callable[_P, _R]: ...

    def tool(self, name: str | _Callable[..., _Any] | None = None, *, description: str | None = None, input_schema: _Mapping[str, _Any] | None = None, output_schema: _Mapping[str, _Any] | None = None) -> _Any:
    ...
    def resource(self, uri: str, *, name: str | None = None, mime_type: str | None = None) -> _Callable[[_Callable[_P, _R]], _Callable[_P, _R]]:
        def register(function: _Callable[_P, _R]) -> _Callable[_P, _R]:
            self._resources[uri] = (_types.Resource(name=name or uri, uri=uri, mime_type=mime_type), function)
            return function
        return register

    def resource_template(self, uri_template: str, *, name: str | None = None, description: str | None = None, mime_type: str | None = None) -> _types.ResourceTemplate:
        """Register a resource template exposed by ``resources/templates/list``."""
        selected = name or uri_template
        template = _types.ResourceTemplate(name=selected, uri_template=uri_template, description=description, mime_type=mime_type)
        self._resource_templates[selected] = template
        return template

    @_overload
    def prompt(self, name: str | None = None, *, description: str | None = None) -> _Callable[[_Callable[_P, _R]], _Callable[_P, _R]]: ...

    @_overload
    def prompt(self, name: _Callable[_P, _R], *, description: str | None = None) -> _Callable[_P, _R]: ...

    def prompt(self, name: str | _Callable[..., _Any] | None = None, *, description: str | None = None) -> _Any:
        def register(function: _Callable[_P, _R]) -> _Callable[_P, _R]:
            selected = name if isinstance(name, str) else function.__name__
            self._prompts[selected] = (_types.Prompt(name=selected, description=description), function)
            return function
        if callable(name) and not isinstance(name, str):
            return register(name)
        return register

    def expect(self, method: str, *, optional: bool = False, repeat: int | tuple[int, int | None] | None = None, subset: bool = False, unordered: str | None = None, fallback: bool = False, **matcher: _Any) -> ExpectedCall:
        expected = ExpectedCall(method, matcher, subset_match=subset, unordered_group=unordered, fallback_call=fallback)
        if optional:
            expected.optional()
        if repeat is not None:
            expected.repeated(repeat, repeat) if isinstance(repeat, int) else expected.repeated(*repeat)
        self._expectations.append(expected)
        return expected

    expect_call = expect

    def expect_tool_call(self, name: str, arguments: _Mapping[str, _Any] | None = None, **options: _Any) -> ExpectedCall:
        return self.expect("tools/call", tool=name, arguments=dict(arguments or {}), **options)

    def _matching(self, method: str, params: _Mapping[str, _Any]) -> ExpectedCall | None:
        candidates = [item for item in self._expectations if item.method == method and (item.maximum is None or item.calls < item.maximum)]
        fallback: ExpectedCall | None = next((item for item in candidates if item.fallback_call), None)
        index = 0
        while index < len(candidates):
            item = candidates[index]
            if item.unordered_group is not None:
                group = [peer for peer in candidates if peer.unordered_group == item.unordered_group]
                for peer in group:
                    actual = {key: params.get(key) for key in peer.matcher}
                    matched = _subset(peer.matcher, actual) if peer.subset_match else _normalize(peer.matcher) == _normalize(actual)
                    if matched:
                        return peer
                if all(peer.optional_call or peer.minimum == 0 or peer.fallback_call for peer in group):
                    index += 1
                    continue
            ...
            matched = _subset(item.matcher, actual) if item.subset_match else _normalize(item.matcher) == _normalize(actual)
            if matched:
                return item
    ...
    def _expect_call(self, method: str, params: _Mapping[str, _Any]) -> ExpectedCall | None:
        expected = self._matching(method, params)
        known = {
            "tools/list", "resources/list", "resources/read", "prompts/list", "prompts/get",
            "resources/templates/list", "resources/subscribe", "resources/unsubscribe", "completion/complete",
            "logging/setLevel",
        }
        registered_tool = method == "tools/call" and isinstance(params.get("tool"), str) and params["tool"] in self._tools
        if expected is None and self.strict and not registered_tool and method not in self._CONTROL_METHODS and method not in known:
    ...
    async def _record(self, method: str, params: _Mapping[str, _Any], response: _Any = None) -> None:
        safe_params = _cast(_Mapping[str, _JsonValue], _redact_for_persistence(dict(params), config=self._redaction_config, path="$.params"))
        safe_response = None if response is None else _redact_for_persistence(response, config=self._redaction_config, path="$.response")
        self._recording.append(RecordedInteraction(
            method, safe_params, _json_safe(safe_response), sequence=self._sequence,
            provenance=("official_mcp_server", "redacted_persistence"),
        ))
        self._sequence += 1

    async def _list_tools(self, _context: _Any, _params: _Any) -> _types.ListToolsResult:
        ...
        if "tools/list" in self.faults.invalid_schema_methods:
            tools = [tool.model_copy(update={"input_schema": {"type": "fault-invalid-schema"}}) for tool in tools]
        if "tools/call" in self.faults.invalid_result_methods:
            tools = [tool.model_copy(update={"output_schema": dict(_INVALID_RESULT_SCHEMA)}) for tool in tools]
        result = expected.response if expected is not None and expected.response is not None else _types.ListToolsResult(tools=tools)
        result = _cast(_types.ListToolsResult, self.faults.response("tools/list", result))
        await self._record("tools/list", request, result)
        return result

    async def _call_tool(self, _context: _Any, params: _types.CallToolRequestParams) -> _types.CallToolResult:
        arguments = dict(params.arguments or {})
        expected = self._expect_call("tools/call", {"tool": params.name, "arguments": arguments})
        ...
        if not isinstance(result, _types.CallToolResult):
            raise TypeError("mock tool handlers must return CallToolResult, text, or a mapping")
        result = _cast(_types.CallToolResult, self.faults.response("tools/call", result))
        await self._record("tools/call", {"tool": params.name, "arguments": arguments}, result)
        return result

    async def _list_resources(self, _context: _Any, _params: _Any) -> _types.ListResourcesResult:
        request = _page_params(_params)
        expected = self._expect_call("resources/list", request)
        result = expected.response if expected is not None and expected.response is not None else _types.ListResourcesResult(resources=[item[0] for item in self._resources.values()])
        result = _cast(_types.ListResourcesResult, self.faults.response("resources/list", result))
        await self._record("resources/list", request, result)
        return result

    async def _read_resource(self, _context: _Any, params: _types.ReadResourceRequestParams) -> _types.ReadResourceResult:
        ...
        result = expected.response if expected is not None and expected.response is not None else await self._invoke(definition[1], params.uri)
        if not isinstance(result, _types.ReadResourceResult):
            result = _types.ReadResourceResult(contents=[_types.TextResourceContents(uri=params.uri, mime_type="text/plain", text=str(result or ""))])
        result = _cast(_types.ReadResourceResult, self.faults.response("resources/read", result))
        await self._record("resources/read", {"uri": params.uri}, result)
        return result

    async def _list_prompts(self, _context: _Any, _params: _Any) -> _types.ListPromptsResult:
        request = _page_params(_params)
        expected = self._expect_call("prompts/list", request)
        result = expected.response if expected is not None and expected.response is not None else _types.ListPromptsResult(prompts=[item[0] for item in self._prompts.values()])
        result = _cast(_types.ListPromptsResult, self.faults.response("prompts/list", result))
        await self._record("prompts/list", request, result)
        return result

    async def _get_prompt(self, _context: _Any, params: _types.GetPromptRequestParams) -> _types.GetPromptResult:
        expected = self._expect_call("prompts/get", {"name": params.name, "arguments": dict(params.arguments or {})})
        ...
        result = expected.response if expected is not None and expected.response is not None else await self._invoke(definition[1], dict(params.arguments or {}))
        if not isinstance(result, _types.GetPromptResult):
            result = _types.GetPromptResult(messages=[_types.PromptMessage(role="user", content=_types.TextContent(text=str(result or "")))])
        result = _cast(_types.GetPromptResult, self.faults.response("prompts/get", result))
        await self._record("prompts/get", {"name": params.name, "arguments": dict(params.arguments or {})}, result)
        return result

    async def _list_resource_templates(self, _context: _Any, _params: _Any) -> _types.ListResourceTemplatesResult:
        request = _page_params(_params)
        expected = self._expect_call("resources/templates/list", request)
        result = expected.response if expected is not None and expected.response is not None else _types.ListResourceTemplatesResult(resource_templates=list(self._resource_templates.values()))
        result = _cast(_types.ListResourceTemplatesResult, self.faults.response("resources/templates/list", result))
        await self._record("resources/templates/list", request, result)
        return result

    async def _subscribe_resource(self, _context: _Any, params: _types.SubscribeRequestParams) -> _types.EmptyResult:
    ...
    async def _unsubscribe_resource(self, _context: _Any, params: _types.UnsubscribeRequestParams) -> _types.EmptyResult:
        self._expect_call("resources/unsubscribe", {"uri": params.uri})
        result = _types.EmptyResult()
        await self._record("resources/unsubscribe", {"uri": params.uri}, result)
        return result

    async def _completion(self, _context: _Any, params: _types.CompleteRequestParams) -> _types.CompleteResult:
        request = {"ref": _json_safe(params.ref), "argument": _json_safe(params.argument)}
    ...
    async def _set_logging_level(self, _context: _Any, params: _types.SetLevelRequestParams) -> _types.EmptyResult:
    ...
    async def _progress(self, _context: _Any, params: _types.ProgressNotificationParams) -> None:
        await self._record("notifications/progress", _cast(_Mapping[str, _Any], _json_safe(params)), {})
    ...
    def server(self) -> _Server:
        return _Server(
            self.name, version=self.version, on_list_tools=self._list_tools, on_call_tool=self._call_tool,
            on_list_resources=self._list_resources, on_list_resource_templates=self._list_resource_templates,
            on_read_resource=self._read_resource, on_subscribe_resource=self._subscribe_resource,
            on_unsubscribe_resource=self._unsubscribe_resource, on_list_prompts=self._list_prompts,
            on_get_prompt=self._get_prompt, on_completion=self._completion,
            on_set_logging_level=self._set_logging_level, on_ping=self._ping,
            on_progress=self._progress, on_roots_list_changed=self._roots_list_changed,
        )
    ...
    def verify(self) -> None:
        missing = [item.method for item in self._expectations if item.calls < item.minimum]
        if missing:
            raise MockExpectationError("missing expected MCP calls: " + ", ".join(missing))

    def recording(self) -> Recording:
        self.verify()
        initialization = _cast(_Mapping[str, _JsonValue], _redact_for_persistence({
            "serverInfo": {"name": self.name, "version": self.version},
            "protocolVersion": "2025-11-25",
        }, config=self._redaction_config, path="$.initialization"))
        return Recording(
            tuple(self._recording), redaction_bound=True,
            provenance=("official_mcp_server", "normalized_decoded_messages"),
            server_name=self.name, server_version=self.version, initialization=initialization,
            redaction_bindings=(RedactionBinding(source="mock_mcp_server", version="1"),),
...
class ReplayServer(MockMCPServer):
    ...
    def __init__(self, recording: Recording, *, strict: bool = True, redaction_config: _RedactionConfig | None = None, artifacts: _Mapping[str, bytes] | None = None, expected_server: tuple[str, str] | None = None) -> None:
        if not recording.redaction_bound:
            raise ValueError("replay requires a recording with an explicit redaction binding")
        selected_config = redaction_config or recording._runtime.config
        safe_recording = recording.redacted(config=selected_config)
        if expected_server is not None and expected_server != (safe_recording.server_name, safe_recording.server_version):
            raise ReplayMismatch("replay server identity differs from the recording")
        if artifacts is not None:
            safe_recording.validate_artifacts(artifacts)
        super().__init__(name=safe_recording.server_name, version=safe_recording.server_version, strict=strict, redaction_config=selected_config)
        ...
        for interaction in self._replay:
            ...
            if interaction.method == "tools/call" and isinstance(raw_name, str):
                name = raw_name
                self._tools.setdefault(name, (_types.Tool(name=name, input_schema={"type": "object"}), None))

    @staticmethod
    def _decoded(interaction: RecordedInteraction, model: _Any, *, fallback: _Any = None) -> _Any:
    ...
    def match(self, method: str, params: _Mapping[str, _Any]) -> RecordedInteraction:
        actual = _normalize(_redact_for_persistence(dict(params), config=self._redaction_config, path="$.replay.params"))
        if self._replay_index >= len(self._replay):
            if self._relaxed:
                self._relaxations.append("relaxed_replay")
                return RecordedInteraction(method, _cast(_Mapping[str, _JsonValue], actual), provenance=("relaxed_replay",))
        ...
        if expected.method != method or _normalize(expected.params) != actual:
            if not self._relaxed:
                raise ReplayMismatch("replay request differs in method, arguments, or ordering")
            self._relaxations.append("relaxed_replay")
            return RecordedInteraction(method, _cast(_Mapping[str, _JsonValue], actual), provenance=("relaxed_replay",))
    ...
    async def _call_tool(self, _context: _Any, params: _types.CallToolRequestParams) -> _types.CallToolResult:
        interaction = self.match("tools/call", {"tool": params.name, "arguments": dict(params.arguments or {})})
        return _cast(_types.CallToolResult, self._decoded(interaction, _types.CallToolResult, fallback=_types.CallToolResult(content=[])))

    async def _list_tools(self, _context: _Any, _params: _Any) -> _types.ListToolsResult:
        return _cast(_types.ListToolsResult, self._decoded(self.match("tools/list", _page_params(_params)), _types.ListToolsResult, fallback=_types.ListToolsResult(tools=[])))

    async def _list_resources(self, _context: _Any, _params: _Any) -> _types.ListResourcesResult:
        return _cast(_types.ListResourcesResult, self._decoded(self.match("resources/list", _page_params(_params)), _types.ListResourcesResult, fallback=_types.ListResourcesResult(resources=[])))

    async def _list_resource_templates(self, _context: _Any, _params: _Any) -> _types.ListResourceTemplatesResult:
        return _cast(_types.ListResourceTemplatesResult, self._decoded(self.match("resources/templates/list", _page_params(_params)), _types.ListResourceTemplatesResult, fallback=_types.ListResourceTemplatesResult(resource_templates=[])))

    async def _read_resource(self, _context: _Any, params: _types.ReadResourceRequestParams) -> _types.ReadResourceResult:
        return _cast(_types.ReadResourceResult, self._decoded(self.match("resources/read", {"uri": params.uri}), _types.ReadResourceResult, fallback=_types.ReadResourceResult(contents=[])))

    async def _list_prompts(self, _context: _Any, _params: _Any) -> _types.ListPromptsResult:
        return _cast(_types.ListPromptsResult, self._decoded(self.match("prompts/list", _page_params(_params)), _types.ListPromptsResult, fallback=_types.ListPromptsResult(prompts=[])))

    async def _get_prompt(self, _context: _Any, params: _types.GetPromptRequestParams) -> _types.GetPromptResult:
        request = {"name": params.name, "arguments": dict(params.arguments or {})}
        return _cast(_types.GetPromptResult, self._decoded(self.match("prompts/get", request), _types.GetPromptResult, fallback=_types.GetPromptResult(messages=[])))

    async def _subscribe_resource(self, _context: _Any, params: _types.SubscribeRequestParams) -> _types.EmptyResult:
        return _cast(_types.EmptyResult, self._decoded(self.match("resources/subscribe", {"uri": params.uri}), _types.EmptyResult, fallback=_types.EmptyResult()))

    async def _unsubscribe_resource(self, _context: _Any, params: _types.UnsubscribeRequestParams) -> _types.EmptyResult:
        return _cast(_types.EmptyResult, self._decoded(self.match("resources/unsubscribe", {"uri": params.uri}), _types.EmptyResult, fallback=_types.EmptyResult()))

    async def _completion(self, _context: _Any, params: _types.CompleteRequestParams) -> _types.CompleteResult:
        request = {"ref": _json_safe(params.ref), "argument": _json_safe(params.argument)}
        return _cast(_types.CompleteResult, self._decoded(self.match("completion/complete", request), _types.CompleteResult, fallback=_types.CompleteResult(completion=_types.Completion(values=[]))))

    async def _set_logging_level(self, _context: _Any, params: _types.SetLevelRequestParams) -> _types.EmptyResult:
        interaction = self.match("logging/setLevel", {"level": params.level})
        return _cast(_types.EmptyResult, self._decoded(interaction, _types.EmptyResult, fallback=_types.EmptyResult()))
    ...
    async def _progress(self, _context: _Any, params: _types.ProgressNotificationParams) -> None:
        self.match("notifications/progress", _cast(_Mapping[str, _Any], _json_safe(params)))
...
__all__ = [
    "ArtifactIntegrityError", "ExpectedCall", "FaultInjector", "Gate", "MockExpectationError",
    "MockMCPServer", "MockProtocolError", "RecordedArtifact", "RecordedInteraction", "Recording", "RedactionBinding",
    "ReplayMismatch", "ReplayServer", "VirtualClock",
]
