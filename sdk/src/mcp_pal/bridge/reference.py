"""A deterministic reference ACP bridge for a structured non-ACP CLI.

This is intentionally a small demonstration bridge, not a generic adapter for
...
keyless E2E; real agents should use their vendor-supported ACP bridge.
...
def _selected_stdio(server: dict[str, Any]) -> subprocess.Popen[str]:
    ...
    return subprocess.Popen(
        ...
        # The bridge does not expose MCP stderr. Avoid an unread pipe that can
...
def run(*, target: str | None = None, target_args: list[str] | None = None) -> int:
    session_id = "reference-" + uuid.uuid4().hex[:12]
    mode_id = "default"
    config: dict[str, Any] = {}
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"reference bridge: {exc}", file=sys.stderr)
        return 1
