...
from .trace.redaction import (
    redact_for_export as _redact_for_export,
)
...
_SnapshotValue: _TypeAlias = (
    bool
    | int
    | float
    | str
    | list["_SnapshotValue"]
    | dict[str, "_SnapshotValue"]
    | None
)
...
class SnapshotOptions:
    ...
    def __post_init__(self) -> None:
        object.__setattr__(
            self,
            "include_fields",
            frozenset(_field_name(item) for item in self.include_fields),
        )
        object.__setattr__(
            self,
            "omitted_fields",
            frozenset(_field_name(item) for item in self.omitted_fields),
        )
...
def _included(key: str, path: str, options: SnapshotOptions) -> bool:
    ...
    return (
        normalized_key in options.include_fields
        or normalized_path in options.include_fields
        or without_snapshot in options.include_fields
    )
...
def _normalize(
    value: _Any, *, path: str, options: SnapshotOptions
) -> _SnapshotValue | None:
    if isinstance(value, _Mapping):
        ...
        for raw_key, raw_value in value.items():
            ...
            if _field_name(key) in options.omitted_fields and not _included(
                key, child_path, options
            ):
                continue
...
def snapshot(
    ...
    *,
    ...
    selected = options or SnapshotOptions(
        ...
        omitted_fields=_DEFAULT_OMITTED_FIELDS
        if omitted_fields is None
        else frozenset(omitted_fields),
    )
