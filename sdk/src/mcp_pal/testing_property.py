...
def _hypothesis() -> Any:
    try:
        ...
        raise ImportError("schema strategies require the 'property' extra: mcp-pal[property]") from None
    return st
...
def _validates(schema: Any, value: Any, root: Any) -> bool:
    ...
    with warnings.catch_warnings():
        ...
        resolver = jsonschema.RefResolver.from_schema(root) if isinstance(root, Mapping) else None
...
def _any_json(st: Any, depth: int = 0) -> Any:
    if depth >= 3:
        return st.one_of(st.none(), st.booleans(), st.integers(-10, 10), st.text(max_size=8))
    child = _any_json(st, depth + 1)
    return st.one_of(
        st.none(), st.booleans(), st.integers(-100, 100),
        st.floats(allow_nan=False, allow_infinity=False, width=32),
        st.text(max_size=16), st.lists(child, max_size=4),
...
def _string_strategy(schema: Mapping[str, Any], st: Any) -> Any:
    ...
    if fmt == "email":
        strategy = st.from_regex(r"[A-Za-z0-9][A-Za-z0-9._%+-]{0,15}@[A-Za-z0-9.-]+\.[A-Za-z]{2,8}", fullmatch=True)
    elif fmt == "ipv4":
        strategy = st.tuples(st.integers(0, 255), st.integers(0, 255), st.integers(0, 255), st.integers(0, 255)).map(lambda parts: ".".join(str(part) for part in parts))
    elif fmt == "ipv6":
        ...
        strategy = st.datetimes(timezones=st.just(timezone.utc)).map(lambda value: value.isoformat().replace("+00:00", "Z"))
    ...
    if isinstance(pattern, str) and fmt is not None:
        try:
            strategy = strategy.filter(lambda value: re.search(pattern, value) is not None)
        except re.error as error:
            raise ValueError("schema string pattern is invalid") from error
    return strategy
...
def _numeric_strategy(schema: Mapping[str, Any], st: Any, *, integer: bool) -> Any:
    ...
    if schema.get("exclusiveMinimum") is not None:
        minimum = schema["exclusiveMinimum"] if minimum is None else max(minimum, schema["exclusiveMinimum"])
        if integer:
            minimum = math.floor(float(minimum)) + 1
    if schema.get("exclusiveMaximum") is not None:
        maximum = schema["exclusiveMaximum"] if maximum is None else min(maximum, schema["exclusiveMaximum"])
        if integer:
    ...
    if multiple is not None:
        ...
        low = math.ceil(float(minimum) / multiple_value) if minimum is not None else -100
        high = math.floor(float(maximum) / multiple_value) if maximum is not None else 100
        if high < low:
            raise ValueError("numeric schema has no values satisfying its bounds")
        strategy = st.integers(low, high).map(lambda value: int(value * multiple_value) if integer else value * multiple_value)
    elif integer:
        strategy = st.integers(min_value=minimum, max_value=maximum)
    else:
        strategy = st.floats(allow_nan=False, allow_infinity=False, min_value=minimum, max_value=maximum, width=32)
...
def _valid_strategy(schema: Any, st: Any, root: Any) -> Any:
    ...
    if isinstance(branches, list) and branches:
        strategy = st.one_of(*[_valid_strategy(branch, st, root) for branch in branches])
    elif isinstance(schema.get("oneOf"), list) and schema["oneOf"]:
        branches = schema["oneOf"]
        strategy = st.one_of(*[_valid_strategy(branch, st, root) for branch in branches])
        strategy = strategy.filter(lambda value: sum(_validates(branch, value, root) for branch in branches) == 1)
    elif isinstance(schema.get("allOf"), list) and schema["allOf"]:
        strategy = _valid_strategy(schema["allOf"][0], st, root).filter(lambda value: _validates(schema, value, root))
    elif "not" in schema:
        strategy = _any_json(st).filter(lambda value: not _validates(schema["not"], value, root))
    else:
        kind = schema.get("type")
        if isinstance(kind, list):
            strategy = st.one_of(*[_valid_strategy({**schema, "type": item}, st, root) for item in kind])
        elif kind == "object":
            properties = schema.get("properties", {})
            required = schema.get("required", [])
            if not isinstance(properties, Mapping) or not isinstance(required, list) or not all(isinstance(name, str) for name in required):
            ...
            required_fields = {name: _valid_strategy(properties[name], st, root) for name in required}
            optional_fields = {name: _valid_strategy(value, st, root) for name, value in properties.items() if name not in required}
            strategy = st.fixed_dictionaries(required_fields, optional=optional_fields)
            additional = schema.get("additionalProperties", True)
            if additional is not False:
                key_strategy = st.text(min_size=1, max_size=8).filter(lambda key: key not in properties)
                additional_schema = additional if isinstance(additional, (Mapping, bool)) else True
                extra = st.dictionaries(key_strategy, _valid_strategy(additional_schema, st, root), max_size=4)
                strategy = st.builds(lambda known, extras: {**extras, **known}, strategy, extra)
            ...
            strategy = st.lists(_valid_strategy(schema.get("items", {}), st, root), min_size=minimum, max_size=maximum)
            if schema.get("uniqueItems"):
                strategy = strategy.filter(lambda value: len({_stable_key(item) for item in value}) == len(value))
        elif kind == "string":
...
def _wrong_type_strategy(kind: str, st: Any) -> Any:
    candidates: dict[str, Any] = {
        "object": st.one_of(st.none(), st.booleans(), st.integers(), st.text(), st.lists(st.none())),
        "array": st.one_of(st.none(), st.booleans(), st.integers(), st.text(), st.dictionaries(st.text(min_size=1, max_size=4), st.none())),
        "string": st.one_of(st.none(), st.booleans(), st.integers(), st.dictionaries(st.text(min_size=1, max_size=4), st.none())),
        "integer": st.one_of(st.none(), st.booleans(), st.text(), st.dictionaries(st.text(min_size=1, max_size=4), st.none())),
        "number": st.one_of(st.none(), st.booleans(), st.text(), st.dictionaries(st.text(min_size=1, max_size=4), st.none())),
...
def invalid_schema_strategy(schema: dict[str, Any] | bool) -> Any:
    ...
    if isinstance(kind, list):
        ...
        strategy = _valid_strategy(schema, st, schema).map(lambda value: {key: item for key, item in value.items() if key != required[0]})
    elif kind == "object" and schema.get("additionalProperties") is False:
        strategy = st.dictionaries(st.text(min_size=1, max_size=8), st.none(), min_size=1, max_size=1)
        ...
        strategy = st.text(max_size=16).filter(lambda value: re.search(pattern, value) is None)
    elif kind in {"integer", "number"} and schema.get("minimum") is not None:
        strategy = st.just(float(schema["minimum"]) - 1 if kind == "number" else int(schema["minimum"]) - 1)
    elif kind in {"integer", "number"} and schema.get("maximum") is not None:
        strategy = st.just(float(schema["maximum"]) + 1 if kind == "number" else int(schema["maximum"]) + 1)
    elif isinstance(kind, str) and kind in {"object", "array", "string", "integer", "number", "boolean", "null"}:
        strategy = _wrong_type_strategy(kind, st)
    else:
        raise ValueError("schema has no supported constraint from which to derive an invalid value")
...
def schema_strategy(schema: dict[str, Any] | bool, *, valid: bool = True) -> Any:
    ...
    if not isinstance(schema, (dict, bool)):
        raise TypeError("schema must be a mapping")
    return _valid_strategy(schema, st, schema) if valid else invalid_schema_strategy(schema)
