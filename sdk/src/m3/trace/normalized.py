...
def _tool_name(value: Any, server: str | None = None) -> str:
    ...
    if name.startswith("mcp__") and len(name.split("__", 2)) == 3:
        name = name.split("__", 2)[2]
    if server and name.lower().startswith(f"{server.lower()}_"):
...
def _call(**values: Any) -> dict[str, Any]:
    ...
    values.setdefault("start_ms", None)
    values.setdefault("end_ms", None)
    values.setdefault("duration_ms", None)
    values.setdefault("server_latency_ms", None)
    values.setdefault("wire_request", None)
    values.setdefault("wire_response", None)
    values.setdefault(
        "provenance",
        {
            "model": True,
            "wire": values["wire_request"] is not None
            or values["wire_response"] is not None,
        },
    )
    return values
...
def from_claude_trace(
    trace: dict[str, Any], selected_server: str, transport: str
) -> list[dict[str, Any]]:
    ...
    intents = [
        ...
        and s.get("metadata", {}).get("mcp_selected") is True
    ]
    # Compatibility for traces built by older callers without classification metadata.
    intents = intents or [
        s
        for s in spans
        if s.get("kind") == "tool_call" and _selected(s.get("name"), selected_server)
    ]
    intents.sort(key=lambda s: float(s.get("start_ms") or 0))
    wire = [
        s for s in spans if s.get("kind") == "mcp" and s.get("name") == "tools/call"
    ]
    used: set[str] = set()
    calls = []
    for index, span in enumerate(intents, 1):
        tool = _tool_name(span.get("name"))
        wire_match = next(
            (
                s
                for s in wire
                if s.get("parent_id") == span.get("id") and str(s.get("id")) not in used
            ),
            None,
        )
        if wire_match is None:
            wire_match = next(
                (
                    s
                    for s in wire
                    if str((s.get("input") or {}).get("params", {}).get("name") or "")
                    == tool
                    and str(s.get("id")) not in used
                ),
                None,
            )
        if wire_match:
            used.add(str(wire_match.get("id")))
        result = span.get("output")
        err = _error(result, span.get("error"))
        if (
            wire_match
            and isinstance(wire_match.get("output"), dict)
            and wire_match["output"].get("error")
        ):
            err = wire_match["output"]["error"]
        call = _call(
            id=str(span.get("id") or f"claude-mcp-{index}"),
            server=selected_server,
            tool=tool,
            status="error"
            if err or span.get("status") in {"error", "failed"}
            else span.get("status", "completed"),
            error=err,
        ...
        if wire_match:
            ...
            call.update(
                arguments=_normalized_arguments(wire_request, call["arguments"]),
                server_latency_ms=wire_match.get("duration_ms"),
                wire_request=wire_request,
                wire_response=wire_match.get("output"),
                provenance={"model": True, "wire": True},
            )
        calls.append(call)
    return calls
...
def _unwrap(event: dict[str, Any]) -> tuple[dict[str, Any], float | None]:
    ...
    return (raw if isinstance(raw, dict) else {}), offset if isinstance(
        offset, (int, float)
    ) else None


def from_opencode_events(
    events: Iterable[dict[str, Any]], selected_server: str, transport: str = "stdio"
) -> list[dict[str, Any]]:
    ...
    for index, wrapper in enumerate(events, 1):
        ...
        duration = (
            ...
            if isinstance(native_start, (int, float))
        ...
        normalized_status = (
            "error"
            if err or native_status == "error"
            else (
                native_status
                if native_status in {"running", "pending"}
                else "completed"
            )
        )
        item = _call(
            id=ident,
            server=selected_server,
            tool=_tool_name(tool_full, selected_server),
        ...
        rank = (
            2
            if item["status"] in {"completed", "error", "failed"}
            and (result is not None or err is not None or duration is not None)
            else 1
        )
        previous = candidates.get(ident)
        if previous is None or rank >= previous[0]:
            candidates[ident] = (rank, item)
...
def build_opencode_trace(
    *,
    ...
    for sequence, record in enumerate(
        sorted(list(protocol_events), key=lambda x: float(x.get("offset_ms", 0) or 0)),
        1,
    ):
        item = dict(record)
    ...
    for record in sorted(protocol, key=lambda x: float(x.get("offset_ms", 0) or 0)):
        ...
        if (
            ...
            if target:
                target["arguments"] = _normalized_arguments(
                    request_payload, target["arguments"]
                )
                target["wire_request"] = request_payload
                target["wire_response"] = payload
                target["server_latency_ms"] = max(
                    0.0,
                    float(record.get("offset_ms", 0) or 0)
                    - float(request.get("offset_ms", 0) or 0),
                )
                target["provenance"]["wire"] = True
                target["limitations"] = [
                    limitation
                    for limitation in target.get("limitations", [])
                    if limitation != UNMATCHED_LIMITATION
                ]
    ...
    spans: list[dict[str, Any]] = [
        {
            ...
            "input": None,
            "output": None,
            "metadata": {"harness": "opencode", "session_id": session_id},
        },
        {
            "id": "run-mcp",
            "parent_id": "run",
            ...
            "status": "completed" if protocol else "unobserved",
            "start_ms": 0.0,
            "end_ms": 0.0,
            "duration_ms": 0.0,
            "transport": transport,
            "input": None,
            "output": None,
            "metadata": {"harness": "opencode", "transport": transport},
        },
    ]
    ...
    for wrapper in raw:
        ...
        if typ == "step_start":
            ...
            turn = ensure_turn(offset)
            turn["end_ms"] = max(float(turn["end_ms"] or 0), offset)
            cost = part.get("cost")
            ...
            turn = ensure_turn(offset)
            kind = "thinking" if typ == "reasoning" else "text"
            if kind == "thinking":
                thinking_count += 1
            span = {
                "id": f"{turn['id']}-{kind}-{len(spans)}",
                "parent_id": turn["id"],
                "kind": kind,
                "name": "Thinking" if kind == "thinking" else "Response",
                "status": "completed",
                "start_ms": offset,
                "end_ms": offset,
                "duration_ms": 0.0,
    ...
    def parent_turn(offset: float) -> dict[str, Any]:
        candidates = [
            turn for turn in turns if float(turn.get("start_ms") or 0) <= offset
        ]
        if candidates:
            return candidates[-1]
    ...
    for call in calls:
        link = wire_links.get(call["id"])
        start = float(call.get("start_ms") or 0)
        end = float(call.get("end_ms") or start)
        if link:
            start = min(start, float(link[0].get("offset_ms", start) or start))
            end = max(end, float(link[1].get("offset_ms", end) or end))
        turn = parent_turn(start)
        turn["end_ms"] = max(float(turn["end_ms"] or 0), end)
        span = {
            "id": f"opencode-tool-{call['id']}",
            "parent_id": turn["id"],
            "kind": "tool_call",
            ...
            "metadata": {
                ...
                "mcp_selected": True,
                "tool_use_id": call["id"],
                "correlation": "wire" if link else "native",
            },
        }
        spans.append(span)
        tool_spans[call["id"]] = span

    protocol_pending: dict[str, tuple[int, dict[str, Any]]] = {}
    protocol_spans: list[dict[str, Any]] = []
    ...
    for sequence, record in enumerate(protocol, 1):
        ...
        if matched_protocol:
            ...
            if method == "tools/call":
                ...
                if queue:
                    matched = queue.pop(0)
                    parent_id = tool_spans.get(matched["id"], {}).get("id", "run-mcp")
            protocol_spans.append(
                {
                    "id": f"opencode-mcp-{request_sequence}",
                    "parent_id": parent_id,
                    "kind": "mcp",
                    "name": method,
                    "status": "error"
                    ...
                    "start_ms": begin,
                    "end_ms": offset,
                    "duration_ms": max(0.0, offset - begin),
                    "transport": transport,
            ...
            protocol_spans.append(
                {
                    "id": f"opencode-mcp-event-{sequence}",
                    "parent_id": "run-mcp",
                    "kind": "mcp_event",
                    "name": str(payload.get("method") or "MCP frame"),
                    "status": "error"
                    if payload.get("error") is not None
                    else "completed",
                    "start_ms": offset,
    ...
    end_ms = max(
        [float(span.get("end_ms") or 0) for span in spans]
        + [float(record.get("offset_ms", 0) or 0) for record in protocol]
    )
    for turn in turns:
        turn["end_ms"] = max(
            float(turn.get("start_ms") or 0), float(turn.get("end_ms") or 0)
        )
        turn["duration_ms"] = max(
            0.0, turn["end_ms"] - float(turn.get("start_ms") or 0)
        )
    spans[0]["end_ms"] = end_ms
    spans[0]["duration_ms"] = end_ms
    if protocol:
        spans[1]["start_ms"] = min(
            float(record.get("offset_ms", 0) or 0) for record in protocol
        )
        spans[1]["end_ms"] = max(
            float(record.get("offset_ms", 0) or 0) for record in protocol
        )
        spans[1]["duration_ms"] = max(0.0, spans[1]["end_ms"] - spans[1]["start_ms"])
    capture = (
        "empty" if not raw else ("complete" if status == "completed" else "partial")
    )
    limitations = (
        []
        if calls and all(c["provenance"].get("wire") for c in calls)
        else ([UNMATCHED_LIMITATION] if calls else [])
    )
    ...
    return {
        ...
        "summary": summary,
        "mcp_calls_schema": SCHEMA_VERSION,
        "mcp_calls": calls,
        "spans": spans,
        "protocol_events": protocol,
        "result_metadata": {"session_id": session_id, "status": status},
