...
This module is intentionally imported lazily by :mod:`m3.storage`.  The
...
class _SqliteBase:
    ...
    def _migrate_suite_uniqueness(connection: _CompatConnection) -> None:
        ...
        try:
            connection.execute("BEGIN IMMEDIATE")
            connection.execute(
                "CREATE TABLE v2_suites_rebuilt ("
                "id INTEGER PRIMARY KEY AUTOINCREMENT, suite_name TEXT NOT NULL, "
                "project_id TEXT REFERENCES v2_projects(id))"
            )
            ...
            if has_project:
                connection.execute(
                    "INSERT INTO v2_suites_rebuilt(id,suite_name,project_id) "
                    "SELECT id,suite_name,project_id FROM v2_suites"
                )
            else:
                connection.execute(
                    "INSERT INTO v2_suites_rebuilt(id,suite_name) "
                    "SELECT id,suite_name FROM v2_suites"
                )
            connection.execute("DROP TABLE v2_suites")
            connection.execute("ALTER TABLE v2_suites_rebuilt RENAME TO v2_suites")
            connection.execute("COMMIT")
        except BaseException:
            try:
                connection.execute("ROLLBACK")
...
class SQLiteExecutionStore(_SqliteBase):
    ...
    def save_test_run(self, run_id: str, value: Mapping[str, object]) -> None:
        ...
        now = _iso(_utcnow())
        # The created_at column is the run's sort key: the manifest start time
        # normalized to UTC, or "" so undated runs sort last.
        sort_key = run_sort_key(safe.get("created_at"))
        connection = self._connect()
        try:
            self._begin(connection, immediate=True)
            connection.execute(
    ...
    def _restore_event(self, value: Mapping[str, Any]) -> Event:
        ...
        marker = event.payload.get("__m3_blob__")
    ...
    def save_evaluation(
        ...
        *,
        ...
        created_at = _utcnow()
        run_id = value.get("run_id") or context.get("metadata", {}).get("m3.run_id")
        if run_id is None:
            snapshot = self.get_snapshot(execution_id)
            run_id = (
