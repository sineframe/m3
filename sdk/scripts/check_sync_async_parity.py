...
def _add_expected(
    expected: dict[str, set[str]], module_name: str, name: str, errors: list[str], label: str
) -> None:
...
def validate_manifest(manifest: dict[str, Any]) -> list[str]:
    ...
    for entry in module_exports.get("paired", ()):
        ...
        _add_expected(expected_exports, async_module, async_name, errors, "paired export")
    ...
    for entry in owner_pairs:
        ...
        _add_expected(expected_exports, async_module, async_name, errors, "owner export")
        if sync_name not in _module_exports(sync_module):
    ...
    for name in module_exports.get("async_pending", ()):
        if not isinstance(name, str):
            ...
            continue
        _add_expected(expected_exports, async_module, name, errors, "pending async export")
        ...
        if name in _module_exports(sync_module):
            errors.append(f"pending async export unexpectedly exists on sync side: {sync_module}.{name}")
    ...
    for name in module_exports.get("sync_only", ()):
        ...
        if name in _module_exports(async_module):
            errors.append(f"sync-only export unexpectedly exists on async side: {async_module}.{name}")
    ...
    for module_name in (sync_module, async_module):
        ...
        for name in sorted(actual - expected):
            errors.append(f"unclassified public export (parity drift): {module_name}.{name}")
    ...
    for key, details in owner_members.items():
        ...
        for name in pending:
            ...
            if hasattr(sync_class, name):
                errors.append(f"pending async member unexpectedly exists on sync side: {sync_spec}.{name}")

        actual_sync = _public_members(sync_class)
        actual_async = _public_members(async_class)
        visible_sync_expected = {name for name in sync_expected if not name.startswith("_")}
        visible_async_expected = {name for name in async_expected if not name.startswith("_")}
...
def main() -> int:
    ...
    try:
        ...
        print(f"sync/async parity check failed: {type(error).__name__}: {error}", file=sys.stderr)
        return 1
    if errors:
