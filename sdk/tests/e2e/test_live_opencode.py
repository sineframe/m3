...
from mcp_pal.types import AgentExecutionSpec, ExecutionOutcome, OpenCode, RestrictiveToolPolicy, SecretReference, ServerBinding, StdioServer, TextContent, TurnOutcome, TurnResponse
...
def _live_spec(executable: str, model: str) -> AgentExecutionSpec:
    ...
    return AgentExecutionSpec(
        harness=OpenCode(model=model, provider=provider, executable=executable, credential_references={"OPENCODE_API_KEY": SecretReference(source="environment", name="OPENCODE_API_KEY")}),
        servers=(ServerBinding(server=StdioServer(name="e2e-mcp", command=sys.executable, args=(str(_MATRIX_SERVER),), cwd=str(_REPOSITORY_ROOT)), alias="e2e-mcp"),),
...
def test_live_spec_helper_is_side_effect_free_and_explicit_about_credentials() -> None:
    ...
    assert spec.harness.credential_references == {
        "OPENCODE_API_KEY": SecretReference(source="environment", name="OPENCODE_API_KEY"),
    }
...
@pytest.mark.skipif(not _LIVE_ENABLED, reason="set MCP_PAL_RUN_LIVE_OPENCODE=1 to call OpenCode")
def test_live_opencode_calls_the_mcp_across_two_turns() -> None:
    ...
    with MCPTestKit(env={}, cwd=str(_REPOSITORY_ROOT)) as kit:
        ...
        assert session.result.trace.events[-1].payload["outcome"] == ExecutionOutcome.COMPLETED.value
