        ...
        servers=(
            ServerBinding(
                server=StdioServer(
                    name="secret-server",
                    command="mcp-server",
                    environment={"SERVICE_TOKEN": SecretReference(source="environment", name="SERVICE_TOKEN")},
                )
            ),
        ),
...
def test_secret_reference_survives_sqlite_process_round_trip(tmp_path: Path) -> None:
    ...
    profile = store.create_server_profile(
        "secret-profile",
        {"mcpServers": {
            "secret-server": {"command": "mcp-server", "env": {"SERVICE_TOKEN": reference}},
            "secret-http": {"type": "http", "url": "https://example.test/mcp", "headers": {"Authorization": reference}},
        }},
    )
    ...
    command = store.enqueue_command(
        execution_id,
        payload={"spec": spec.model_dump(mode="json"), "bearer_token": reference, "note": canary},
    )
    ...
    store.append_events([EventFactory(execution_id).create(EventKind.DIAGNOSTIC, payload={"note": canary * 4000})])
...
""",
    ...
    assert observed == {"spec": expected, "profile": expected, "http": expected, "harness": expected, "command": expected, "clone": expected, "canary_redacted": True, "canary_leaked": False}


def test_durable_credential_literals_are_rejected_instead_of_redacted(tmp_path: Path) -> None:
    store = SQLiteExecutionStore(
        tmp_path / "reject-secret.sqlite",
        blob_root=tmp_path / "blobs",
        config=RedactionConfig(sensitive_keys=frozenset({"private_marker"}), include_environment=False),
    )
    with pytest.raises(DurableSerializationError, match="requires a SecretReference"):
        store.create_server_profile(
            "literal-profile",
            {"mcpServers": {"server": {"type": "http", "url": "https://example.test", "headers": {"Authorization": "literal-token"}}}},
        )
    with pytest.raises(DurableSerializationError, match="requires a SecretReference"):
        store.create_server_profile("configured-key", {"private_marker": "literal-token"})
    configured = store.create_server_profile(
        "configured-reference", {"private_marker": SecretReference(source="environment", name="PRIVATE_MARKER")}
    )
    ...
    nested = store.create_harness_profile(
        ...
        {"credentials": {"provider": {
            "api_key": SecretReference(source="provider", name="provider-api-key"),
            "bearer_token": SecretReference(source="provider", name="provider-bearer"),
        }}},
    )
...
@pytest.mark.parametrize("boundary", ["specification", "server_bindings", "harness_binding"])
def test_durable_credential_rejection_is_atomic(tmp_path: Path, boundary: str) -> None:
    store = SQLiteExecutionStore(tmp_path / f"atomic-{boundary}.sqlite", blob_root=tmp_path / f"blobs-{boundary}")
    execution_id = ExecutionId(f"atomic-{boundary}")
    kwargs = {
        "specification": {"headers": {"Authorization": "literal-token"}},
        "server_bindings": [{"server": {"headers": {"Authorization": "literal-token"}}}],
...
@pytest.mark.parametrize("value", ["literal-secret ${IGNORED}", "prefix-${TOKEN}", "Bearer ${TOKEN}-suffix", "bearer ${TOKEN}"])
def test_legacy_placeholder_bypass_rejects_literal_suffixes(tmp_path: Path, value: str) -> None:
    store = SQLiteExecutionStore(tmp_path / "placeholder.sqlite", blob_root=tmp_path / "blobs")
    with pytest.raises(DurableSerializationError, match="requires a SecretReference"):
        store.create_server_profile("placeholder", {"headers": {"Authorization": value}})
    store.close()


@pytest.mark.parametrize("value", ["${TOKEN}", "Bearer ${TOKEN}"])
def test_legacy_placeholder_forms_are_explicitly_accepted(tmp_path: Path, value: str) -> None:
    store = SQLiteExecutionStore(tmp_path / "accepted-placeholder.sqlite", blob_root=tmp_path / "blobs")
    profile = store.create_server_profile("placeholder", {"headers": {"Authorization": value}})
...
def test_durable_urls_reject_embedded_credentials(tmp_path: Path, url: str) -> None:
    store = SQLiteExecutionStore(tmp_path / "credential-url.sqlite", blob_root=tmp_path / "blobs")
    with pytest.raises(DurableSerializationError, match="URL cannot contain credentials"):
        store.create_server_profile("credential-url", {"url": url})
    store.close()


def test_durable_serialization_rejects_cycles_and_excessive_nesting_atomically(tmp_path: Path) -> None:
