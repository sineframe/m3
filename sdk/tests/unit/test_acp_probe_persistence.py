...
def _result(dimension: ACPProbeDimension, *, probe_id: str, status: ACPProbeStatus, created_at: datetime) -> ACPProbeResult:
...
def test_store_latest_is_exact_dimension_and_sqlite_reopens(tmp_path: Path, store_factory: object) -> None:
    store = store_factory() if store_factory is InMemoryExecutionStore else store_factory(tmp_path / "probes.sqlite")  # type: ignore[operator]
    ...
    dimension = ACPProbeDimension(profile_id=profile_id, revision_id=revision_id, probe_type=ACPProbeKind.FULL, transport="stdio", agent_mode_id="m", session_config={"x": 1})
    now = datetime.now(timezone.utc)
    store.save_acp_probe(_result(dimension, probe_id="older", status=ACPProbeStatus.VERIFIED, created_at=now))
    store.save_acp_probe(_result(dimension, probe_id="newer", status=ACPProbeStatus.FAILED, created_at=now + timedelta(seconds=1)))
    other = dimension.model_copy(update={"transport": "http"})
    store.save_acp_probe(_result(other, probe_id="other", status=ACPProbeStatus.VERIFIED, created_at=now + timedelta(seconds=2)))
    with pytest.raises(StorageConflict):
        store.save_acp_probe(_result(other, probe_id="older", status=ACPProbeStatus.VERIFIED, created_at=now))
...
def test_runner_whitelists_output_and_preserves_request_identity() -> None:
    request = ACPProbeRequest(profile_id="p", revision_id="r", probe_type=ACPProbeKind.PROTOCOL, timeout_seconds=1)

    async def runner(_request: ACPProbeRequest) -> dict[str, JsonValue]:
        return {"status": "queued", "profile_id": "evil", "id": "evil", "created_at": "evil", "evidence": {"safe": True}}

    result = asyncio.run(run_acp_probe(request, runner))
    assert result.status is ACPProbeStatus.FAILED
    assert result.profile_id == "p" and result.revision_id == "r" and result.id != "evil"
    assert result.created_at != "evil"
    assert result.evidence == {"safe": True}


def test_runner_timeout_and_redaction() -> None:
    request = ACPProbeRequest(profile_id="p", revision_id="r", probe_type=ACPProbeKind.PROTOCOL, timeout_seconds=0.01)
...
def test_session_secret_is_rejected_before_write(tmp_path: Path, store_kind: str) -> None:
    config = RedactionConfig(secrets=frozenset({"session-canary"}), include_environment=False)
    store = InMemoryExecutionStore(config=config) if store_kind == "memory" else SQLiteExecutionStore(tmp_path / "secret.sqlite", config=config)
    if isinstance(store, SQLiteExecutionStore):
        profile = store.create_harness_profile("agent", {"manifest": {"command": "echo"}})
        profile_id, revision_id = profile.id, str(store.resolve_revision(profile.id).id.root)
    else:
        profile_id, revision_id = "p", "r"
    result = _result(ACPProbeDimension(profile_id=profile_id, revision_id=revision_id, probe_type=ACPProbeKind.FULL, session_config={"quality": "session-canary"}), probe_id="secret", status=ACPProbeStatus.VERIFIED, created_at=datetime.now(timezone.utc))
