...
from mcp_pal.configuration import ConfigSource, ConfigError, Config, load_config
...
def test_direct_construction_infers_truthful_provenance() -> None:
    config = Config(artifact_policy="always", protocol_revision="2025-06-18", telemetry_enabled=True)
...
def test_all_four_precedence_levels_and_origins(tmp_path: Path) -> None:
    ...
    config = load_config({"artifact_policy": "failed"}, env={"MCP_PAL_ARTIFACT_POLICY": "never", "MCP_PAL_PROTOCOL_REVISION": "2025-03-26"}, cwd=tmp_path, telemetry_enabled=False)
...
def test_stable_environment_names_and_boolean_validation() -> None:
    config = load_config(env={"MCP_PAL_ARTIFACT_POLICY": "always", "MCP_PAL_PROTOCOL_REVISION": "auto", "MCP_PAL_TELEMETRY_ENABLED": "true"}, cwd=Path("/tmp/mcp-pal-no-project"))
...
def test_unknown_settings_are_rejected_without_echoing_values(tmp_path: Path) -> None:
    ...
    with pytest.raises(ConfigError):
        load_config(env={"MCP_PAL_UNKNOWN": "TOP-SECRET"}, cwd=Path("/tmp/mcp-pal-no-project"))


def test_configuration_json_round_trip_and_aliases() -> None:
    config = load_config({"artifact_policy": "never", "protocol_revision": "2025-06-18", "telemetry_enabled": True}, env={}, cwd=Path("/tmp/mcp-pal-no-project"))
    restored = Config.model_validate(json.loads(json.dumps(config.model_dump(mode="json"))))
