...
def test_profile_readiness_check_reports_missing_references(monkeypatch):
    cfg={"mcpServers":{"draw":{"command":"node","env":{"TOKEN":"${DRAW_TOKEN}"}}}}
...
def test_profile_shape_validation_does_not_touch_environment(monkeypatch):
    monkeypatch.setattr(validation, "_check_refs", lambda *_: pytest.fail("shape validation inspected os.environ"))
    cfg={"mcpServers":{"draw":{"command":"node","env":{"TOKEN":"${DRAW_TOKEN}"}}}}
