...
SCRIPT = Path(__file__).parents[3] / "scripts" / "dev_db_reset.py"
...
def test_reset_requires_exact_confirmation_and_removes_only_sidecars(tmp_path):
    ...
    assert db.exists()
    result = _run(tmp_path, env={"CONFIRM": "reset", "MCP_PAL_DATABASE_PATH": "state.sqlite"})
...
def test_reset_rejects_outside_path_and_directories(tmp_path):
    ...
    result = _run(tmp_path, env={"CONFIRM": "reset", "MCP_PAL_DATABASE_PATH": str(outside)})
    assert result.returncode == 2
    assert outside.exists()
    directory = tmp_path / "database-dir"
    directory.mkdir()
    result = _run(tmp_path, env={"CONFIRM": "reset", "MCP_PAL_DATABASE_PATH": "database-dir"})
    assert result.returncode == 2
...
def test_hostile_confirmation_is_data_not_shell(tmp_path):
    ...
    result = _run(
        ...
        env={
            ...
            "MCP_PAL_DATABASE_PATH": "state.sqlite",
        },
    )
    assert result.returncode == 2
...
def test_reset_rejects_symlinked_database_and_sidecars(tmp_path):
    ...
    result = _run(tmp_path, env={"CONFIRM": "reset", "MCP_PAL_DATABASE_PATH": database.name})
    assert result.returncode == 2
...
def test_reset_rejects_extra_arguments_even_with_environment_confirmation(tmp_path):
    ...
    result = _run(tmp_path, "ignored", env={"CONFIRM": "reset", "MCP_PAL_DATABASE_PATH": database.name})
    assert result.returncode == 2
