...
def _launch(command: str) -> HarnessLaunch:
    ...
    capture = SimpleNamespace(
        enforces_portable_policy=lambda values: (
            tuple(values) == ("contract-policy-proxy",)
        )
    )
...
async def test_acp_adapter_keeps_one_session_across_turns(tmp_path: Path) -> None:
    ...
    assert (
        first.evidence["session_id"]
        == second.evidence["session_id"]
        == "persistent-session"
    )
...
async def test_acp_typed_observations_and_public_runtime_are_finalized(
    ...
    for entry in view.timeline:
        if entry.kind != "provider" or getattr(entry, "category", "") != "raw_frame":
            continue
        if entry.data.state is ObservationState.OBSERVED and isinstance(
            entry.data.value, Mapping
        ):
    ...
    assert raw_timing
    assert min(
        offset for method, offset in raw_timing if method == "initialize"
    ) <= min(offset for method, offset in raw_timing if method == "session/prompt")


@pytest.mark.asyncio
async def test_acp_tool_observation_preserves_builtin_identity_and_result(
    tmp_path: Path,
) -> None:
    ...
    results = [
        item
        for item in result.turn_evidence.observations
        if item.kind == "tool_result_observed"
    ]
...
@pytest.mark.asyncio
async def test_acp_applies_typed_mode_and_config_before_first_prompt(
    tmp_path: Path,
) -> None:
    command = _configured_agent(tmp_path / "configured-agent.py")
    launch = _launch(command)
    spec = launch.spec.model_copy(
        update={
            "harness": ACPAgent(
                model="fixture",
                manifest={"command": command, "protocol": "acp", "protocol_version": 1},
    ...
    methods = [
        frame["payload"].get("method")
        for frame in frames
        if isinstance(frame.get("payload"), dict)
    ]
    assert (
        methods.index("session/set_mode")
        < methods.index("session/set_config_option")
...
@pytest.mark.asyncio
async def test_acp_public_runtime_projects_modes_and_selected_config(
    tmp_path: Path,
) -> None:
...
@pytest.mark.asyncio
async def test_acp_public_source_shape_preserves_message_reasoning_plan_and_state(
    tmp_path: Path,
) -> None:
...
async def test_acp_rejects_stale_typed_session_option(tmp_path: Path) -> None:
    ...
    launch = HarnessLaunch(
        spec, base.servers, base.configurations, spec.tool_policy, capture=base.capture
    )
...
@pytest.mark.asyncio
async def test_acp_native_agent_default_policy_is_explicitly_nonportable(
    tmp_path: Path,
) -> None:
    del tmp_path
    ...
    policy = NativeToolPolicy(
        harness="acp",
        policy={"mode": "agent_default", "server": "selected"},
    ...
    launch = HarnessLaunch(
        ...
        ServerGroupSnapshot(
            (
                ServerRecord(
                    "selected", server, True, True, "selected", TransportKind.STDIO
                ),
            )
        ),
        (
...
@pytest.mark.asyncio
async def test_acp_policy_capability_requires_proxy_proof_and_rejects_native_policy() -> (
    None
):
...
async def test_acp_adapter_rejects_credential_query_and_unresolved_secret() -> None:
    ...
    query_spec = AgentSpec(
        ...
        servers=(
            ServerBinding(
                server=HTTPServer(
                    name="remote", url="https://example.test/mcp?token=canary"
                )
            ),
        ),
    )
    query_launch = HarnessLaunch(
        query_spec,
        ServerGroupSnapshot(),
        (
            HarnessServerConfig(
                ...
                available=True,
                connection_id="c",
                endpoint="https://example.test/mcp?token=canary",
            ),
        ),
        ...
        servers=(
            ServerBinding(
                server=StdioServer(
                    ...
                    command="echo",
                    environment={
                        "TOKEN": SecretReference(source="environment", name="TOKEN")
                    },
                )
            ),
        ),
    )
    secret_launch = HarnessLaunch(
        secret_spec,
        ServerGroupSnapshot(),
        (
            HarnessServerConfig(
    ...
    assert (
        await AcpHarnessAdapter().preflight(query_launch)
    ).reason == "credential_query_unsupported"
    assert (
        await AcpHarnessAdapter().preflight(secret_launch)
    ).reason == "credential_unavailable"


@pytest.mark.asyncio
async def test_default_registry_runs_three_turn_public_agent_session(
    tmp_path: Path,
) -> None:
    ...
    spec = AgentSpec(
        ...
        servers=(
            ...
            ServerBinding(
                server=StdioServer(name="second", command="echo"), required=False
            ),
        ),
    )
    async with AsyncMCPTestKit(env={}, cwd=tmp_path) as kit:
        ...
        async with session:
            ...
            assert all(
                result.evidence["usage_requested"] is False for result in results
            )
            ...
            assert all(
                result.evidence["usage_unavailable"] is True for result in results
            )
...
@pytest.mark.asyncio
async def test_acp_transport_rejects_huge_unterminated_frame_before_allocation(
    tmp_path: Path,
) -> None:
