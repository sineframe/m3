...
def _in_process_binding() -> InProcessServer:
    async def list_tools(_context: Any, params: Any) -> types.ListToolsResult:
        ...
        return types.ListToolsResult(tools=[tool], next_cursor=None if cursor else "page-2")
...
def test_sync_direct_local_contract_returns_only_sync_values(binding_factory: Any) -> None:
        ...
        with MCPTestKit(env={}, cwd=str(_ROOT)) as outer:
            with MCPTestKit(env={}, cwd=str(_ROOT)) as inner:
                with cast(Any, inner.direct(_in_process_binding())) as client:
                    assert client.call_tool("echo", {"text": "nested"}).content[0]["text"] == "nested"
            inner.close()
            outer.close()
        outer.close()
    leaked = {
...
def test_twenty_concurrent_direct_portals_are_reaped() -> None:
        ...
        try:
            ...
            with MCPTestKit(env={}, cwd=str(_ROOT)) as kit:
                with cast(Any, kit.direct(_in_process_binding())) as client:
                    assert client.call_tool("echo", {"text": "parallel"}).content[0]["text"] == "parallel"
        except BaseException as error:
            errors.append(error)

    threads = [threading.Thread(target=worker, name=f"mcp-pal-sync-{index}") for index in range(20)]
    ...
    while time.monotonic() < deadline:
        active = {thread.ident for thread in threading.enumerate() if thread.ident is not None}
        if active <= baseline:
            break
        threading.Event().wait(0.01)
    assert {thread.ident for thread in threading.enumerate() if thread.ident is not None} <= baseline
...
def test_invalid_protocol_fails_before_startup_without_portal_leak() -> None:
    ...
    try:
        with pytest.raises(UnsupportedFeature):
            with cast(Any, kit.direct(_in_process_binding(), protocol=ProtocolConstraint(revision="2024-11-05"))):
                pass
    finally:
        kit.close()
...
def test_concurrent_close_calls_are_idempotent_and_deadlock_free() -> None:
    ...
    threads = [threading.Thread(target=close, name=f"mcp-pal-close-{index}") for index in range(20)]
    for thread in threads:
        thread.start()
    for thread in threads:
