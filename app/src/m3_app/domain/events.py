...
def normalize_events(
    ...
    if typ in ("step_start", "step_finish", "text", "reasoning", "tool_use"):
        ...
        if typ == "step_start":
            return [("step_start", {**common, "step": part})]
        if typ == "step_finish":
            return [
                (
                    "step_finish",
        ...
        server_name = (
            selected_server
            if selected_server
            and name.lower().startswith(selected_server.lower() + "_")
            else None
        )
        call = (
            "tool_call",
    ...
    if typ == "stream_event":
        ...
        if event_type == "content_block_start":
            ...
            if block_type == "thinking":
                return [("thinking", {"content": block, "partial": True, "raw": raw})]
        ...
        if event_type == "content_block_delta":
            ...
            if delta_type == "text_delta":
                return [
                    (
                        "assistant_text",
                        {"text": delta.get("text", ""), "partial": True, "raw": raw},
                    )
                ]
            if delta_type == "input_json_delta":
                return [
                    ("tool_call", {"content": [delta], "partial": True, "raw": raw})
                ]
    ...
    if typ in ("assistant", "message"):
        ...
        if isinstance(content, str):
            return [("assistant_text", {"text": content, "raw": raw})]
        if isinstance(content, list):
            out = []
            for block in content:
                if not isinstance(block, dict):
                    out.append(("assistant_text", {"text": str(block), "raw": raw}))
                    continue
                btype = block.get("type")
                if btype == "thinking":
                    out.append(("thinking", {"content": block, "raw": raw}))
                elif btype in ("tool_use", "tool_call"):
                    name = block.get("name", "")
                    server_name = (
                        selected_server
                        if selected_server
                        and f"mcp__{selected_server.lower()}__" in name.lower()
                        else None
                    )
                    out.append(
                        (
                    ...
                    out.append(
                        (
                            "assistant_text",
                            {"text": block.get("text", ""), "content": block},
                        )
                    )
    ...
    if typ == "system":
        return [
            (
                "mcp_initialization"
                if "init" in subtype or "mcp" in str(raw).lower()
                else "system",
                raw,
            )
        ]
    if typ in ("tool_result", "tool_response") or subtype in (
        "tool_result",
        "tool_response",
    ):
        return [("tool_result", raw)]
    if typ in ("result", "final"):
        return [
            (
                "assistant_text",
                {"text": raw.get("result", raw.get("text", "")), "raw": raw},
            )
        ]
    if "retry" in subtype or typ == "api_retry":
        return [("api_retry", raw)]
    if typ == "error" or "error" in subtype:
        return [("error", raw)]
    if typ == "user" and raw.get("tool_use_result") is not None:
        return [("tool_result", raw)]
    return [("system", raw)]
...
def _namespace(server: str) -> str:
    return f"mcp__{server.lower()}__"
...
def _call_info(payload: JSON) -> tuple[str | None, str]:
    ...
    return (
        block.get("id") or payload.get("tool_use_id"),
        _name(block.get("name") or payload.get("tool_name") or payload.get("name")),
    )


def _result_error(payload: Any) -> bool:
    if isinstance(payload, dict):
        if payload.get("is_error") or payload.get("tool_error"):
            return True
        ...
        return any(
            ...
            if k in ("tool_use_result", "result", "content", "message")
        )
    if isinstance(payload, list):
        return any(_result_error(v) for v in payload)
    return False
...
def _normalized(raw_events: list[Any], server: str) -> list[tuple[str, JSON]]:
    ...
    for raw in raw_events:
        try:
            key = json.dumps(raw, sort_keys=True, ensure_ascii=False)
        except TypeError:
            key = repr(raw)
        if key in seen:
            continue
...
def derive_mcp_summary(raw_events: list[Any], selected_server: str) -> JSON:
    ...
    for raw in raw_events:
        ...
        if isinstance(servers, dict):
            ...
            if isinstance(entry, dict):
                state = str(entry.get("status", entry.get("state", ""))).lower()
            else:
                state = str(entry or "").lower()
        elif isinstance(servers, list):
            entry = next(
                (
                    item
                    for item in servers
                    if isinstance(item, dict) and item.get("name") == selected_server
                ),
                None,
            )
            state = str(
                (entry or {}).get("status", (entry or {}).get("state", ""))
            ).lower()
        else:
            continue
        if state in ("connected", "ready", "initialized", "ok", "success"):
        ...
        if isinstance(raw, dict):
            for key in ("tools", "mcp_tools", "advertised_tools"):
                ...
                if isinstance(value, list):
                    advertised.extend(
                        _name(x.get("name") if isinstance(x, dict) else x)
                        for x in value
                    )
    calls = []
    call_ids = set()
    ...
    for typ, payload in normalized:
        ...
        if (
            ns in name.lower()
            or name.lower().startswith(selected_server.lower() + "_")
            or payload.get("server_name") == selected_server
            or payload.get("server") == selected_server
        ):
            call_key = (
                ("id", ident)
                if ident
                    ...
                    name,
                    json.dumps(
                        payload.get("content", []), sort_keys=True, ensure_ascii=False
                    ),
                )
            )
            ...
            calls.append({"tool_use_id": ident, "name": name})
            call_ids.add(ident) if ident else None
    matched_results = []
    unmatched = []
    for typ, payload in normalized:
        if typ != "tool_result":
            continue
        ident, name = _call_info(payload)
        if ident and ident in call_ids:
            matched_results.append(payload)
        elif not ident and ns in name.lower():
            matched_results.append(payload)
        else:
            unmatched.append(payload)
    errors = sum(1 for p in matched_results if _result_error(p))
    successes = len(matched_results) - errors
    advertised = list(dict.fromkeys(x for x in advertised if x))
    return {
        "initialization_state": init_state,
        "advertised_tools": advertised,
        "selected_server_call_count": len(calls),
        "selected_server_call_names": [x["name"] for x in calls],
        "success_count": successes,
        "error_count": errors,
        "correlated_result_count": len(matched_results),
    }
...
def derive_mcp_assertion(raw_events: list[Any], selected_server: str) -> str:
    ...
    if (
        summary["selected_server_call_count"] == 0
        or summary["correlated_result_count"] == 0
    ):
        return "failed"
    if summary["success_count"] and summary["error_count"]:
        return "warning"
