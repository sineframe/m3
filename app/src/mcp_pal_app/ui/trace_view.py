...
from mcp_pal.trace.redaction import RedactionConfig, redact_for_ui
...
def display_name(span: dict[str, Any]) -> str:
    ...
    if kind == "thinking":
        ...
        return "Thinking" + (
            f" · {availability}"
            if availability in {"empty", "encrypted", "omitted"}
            else ""
        )
    if kind == "text":
        return "Response"
    if kind == "tool_call":
        prefix = (
            "MCP" if (span.get("metadata") or {}).get("mcp_selected", True) else "Tool"
        )
...
def actor(span: dict[str, Any]) -> str:
    ...
    if kind == "tool_call":
        return (
            "MCP call"
            if (span.get("metadata") or {}).get("mcp_selected", True)
            else f"{agent} tool"
        )
    if kind in PROTOCOL_KINDS:
        return "MCP server"
...
def visible_spans(
    spans: list[dict[str, Any]], *, include_protocol: bool = False
) -> list[dict[str, Any]]:
    source = list(spans or [])
...
def server_latency_for(
    span: dict[str, Any], spans: list[dict[str, Any]]
) -> float | None:
    children = [
        item
        for item in spans
        if item.get("parent_id") == span.get("id")
        and item.get("kind") == "mcp"
        and item.get("name") == "tools/call"
    ]
    if not children:
        return None
...
def preview_value(value: Any, *, config: RedactionConfig | None = None) -> Any:
    """Return the UI-safe projection used by preview/view-model consumers."""
    return redact_for_ui(value, config=config)
