...
def _saved_opencode_auth(executable, providers=None):
    try:
        ...
        if providers is None:
            return "credentials" in text and any(
                line.strip()
                and not line.lower().startswith(("credentials", "┌", "└", "│"))
...
def _opencode_provider_env(provider, settings):
    ...
    return bool(
        configured
        or os.environ.get(f"{normalized}_API_KEY")
        or os.environ.get(f"{normalized}_AUTH_TOKEN")
    )
...
def _manifest_local_ready(manifest):
    ...
    try:
        ...
        return False, [], None
    return (
        bool(checked.get("local_ready")),
        checked.get("missing_environment", []),
        checked.get("executable"),
    )
...
def _model_option_value(config_options, session_config):
    ...
    for option in config_options:
        if (
            ...
            or str(option.get("category", "")).lower() != "model"
        ):
            continue
        ident = option.get("id") or option.get("configId") or option.get("key")
        if (
            ident in session_config
...
def _latest_protocol_probe(db, revision_id):
    return (
        ...
        .order_by(desc(HarnessProbe.created_at))
        .first()
    )
...
def _latest_full_probe(db, revision_id, transport, mode_id, session_config):
    ...
    probes = (
        db.query(HarnessProbe)
        .filter_by(
            revision_id=revision_id, kind="full", transport=transport, mode_id=mode_id
        )
        .order_by(desc(HarnessProbe.created_at))
        .all()
    )
    ...
    return next(
        (
            ...
            for probe in probes
            if _json.dumps(
                probe.session_config or {}, sort_keys=True, separators=(",", ":")
            )
            == wanted
        ),
        None,
...
def _verification_for(db, revision, *, transport, mode_id, session_config):
    ...
    return {
        ...
        "full_probe_status": "identity_mismatch"
        if identity_mismatch
        else (full.status if full else "unverified"),
        "full_probe_dimensions": {
            "transport": transport,
            "mode_id": mode_id,
...
def _config_option_list(value):
    if isinstance(value, dict):
        ...
        value = (
            value.get("options")
            or value.get("configOptions")
            or value.get("config_options")
            or value.get("availableOptions")
            or []
        )
...
def _protocol_defaults(protocol):
    ...
    mode = (
        (raw_modes.get("currentModeId") or raw_modes.get("current_mode_id"))
...
def _validate_acp_dimensions(db, revision, *, mode_id, session_config):
    ...
    if not protocol or protocol.status != "verified":
        if mode_id is not None or config:
            raise HTTPException(
                422, "Unprobed ACP runs require empty mode and session configuration"
            )
        return
    ...
    if mode_id is not None and (not modes or str(mode_id) not in modes):
        raise HTTPException(
            422, "ACP mode is not advertised by the latest protocol probe"
        )
    options = _config_option_list(evidence.get("config_options"))
    option_map = {
        str(x.get("id") or x.get("configId")): x
        for x in options
        if isinstance(x, dict) and (x.get("id") or x.get("configId")) is not None
    }
    unknown = sorted(set(config) - set(option_map))
    if unknown:
        raise HTTPException(
            422,
            "ACP session configuration contains unadvertised options: "
            + ", ".join(unknown),
        )
    ...
    if missing:
        raise HTTPException(
            422,
            "ACP session configuration must include every advertised option: "
            + ", ".join(missing),
        )
    for ident, value in config.items():
        ...
        if option_type in {"boolean", "bool"}:
            ...
            option_type in {"select", "enum", "string"}
            or option.get("options") is not None
        ):
            ...
            if not any(
                ...
                raise HTTPException(
                    422, f"ACP session option {ident} has an unadvertised value"
                )
        elif option_type:
            raise HTTPException(
                422, f"Unsupported ACP session option type: {option_type}"
            )
...
def profile_json(p: McpProfile, include_json=False):
        ...
        "revisions": [
            {
                ...
                **({"mcp_json": r.mcp_json} if include_json else {}),
            }
            for r in revisions
        ],
    }
    if include_json and revisions:
        current = next(
            (r for r in revisions if r.id == p.current_revision_id), revisions[-1]
        )
        try:
            out["validation"] = validate_mcp_config(current.mcp_json)
...
def create_app(
    ...
    *,
    ...
    factory = session_factory or __import__(
    ...
    app.state.settings, app.state.session_factory, app.state.manager = (
        settings,
        factory,
        manager,
    )
    ...
    def health():
        ...
        db_ok = True
        try:
            d.execute(__import__("sqlalchemy").text("SELECT 1"))
        except Exception:
            db_ok = False
        finally:
            d.close()
    ...
    def capabilities(d: Session = Depends(db_dep)):  # noqa: B008 - FastAPI dependency/body marker
        ...
        opencode_flags, _, opencode_executable = _probe(
            settings.opencode_executable, REQUIRED_OPENCODE_FLAGS, ("run",)
        )
        providers = settings.opencode_providers()
        opencode_auth = any(
        ...
        claude_ready = bool(
            settings.anthropic_api_key and claude_executable and claude_flags
        )
        ...
        for p in (
            d.query(HarnessProfile)
            .filter_by(archived=False)
            .order_by(HarnessProfile.name)
            .all()
        ):
            from mcp_pal_app.persistence.models import HarnessProbe

            probes = (
                d.query(HarnessProbe)
                .filter_by(revision_id=p.current_revision_id)
                .order_by(desc(HarnessProbe.created_at))
                .all()
            )
            ...
            for candidate in dimensions.values():
                ...
                mismatch = bool(
                    protocol_identity
                    and candidate.agent_identity
                    and candidate.agent_identity != protocol_identity
                )
                if mismatch:
                    identity_mismatches.append(candidate)
                else:
                    verified_dimensions.append(candidate)
            ...
            _listed_ids(raw_modes, "availableModes", "available_modes")
            mode_descriptors = (
                (
                ...
                if isinstance(raw_modes, dict)
                else (raw_modes if isinstance(raw_modes, list) else [])
            )
            current_mode_id = (
                (raw_modes.get("currentModeId") or raw_modes.get("current_mode_id"))
                if isinstance(raw_modes, dict)
            ...
            full_verifications = [
                {
                    ...
                    "mode_id": x.mode_id,
                    "session_config": x.session_config,
                    "status": x.status,
                    "agent_identity": x.agent_identity,
                    "identity_match": x in verified_dimensions,
                    "evidence": x.evidence,
                }
            ...
            missing_identity = bool(
                full
                and full.status == "verified"
                and (full.agent_identity is None or protocol_identity is None)
            )
            probe_states = {
                "protocol": protocol.status if protocol else "unverified",
                "full": full.status if full else "unverified",
            }
            warnings = (
                (
                    ["Identity changed; full verification downgraded"]
                    if identity_warning
                    else []
                )
                + (
                    ["Agent identity unavailable; verification limited"]
                    if missing_identity
                    else []
                )
                + (
                    []
                    if full and full.status == "verified"
                    else ["Harness is not fully verified"]
                )
            )
            ...
            out["harnesses"].append(
                {
                    "selection_id": f"profile:{p.id}",
                    "kind": "acp",
                    "harness": "acp",
                    "name": p.name,
                    "profile_id": p.id,
                    ...
                    "agent_modes": mode_descriptors,
                    "current_agent_mode_id": current_mode_id,
                    ...
                    "full_verified": bool(full and full.status == "verified"),
                    "full_verifications": full_verifications,
                    "verification": {
                        "protocol": protocol_evidence or None,
                        "full": full.evidence if full else None,
                    },
                    "warnings": warnings,
                }
            )
                ...
                "revisions": [
                    {
                        ...
                        "revision_number": r.revision_number,
                        "manifest": r.manifest,
                        "trusted_unsandboxed": r.trusted_unsandboxed,
                    }
    ...
    def add_harness_revision(
        ...
        if not body.trusted_unsandboxed:
            raise HTTPException(422, "Trusted unsandboxed acknowledgment is required")
        n = max([r.revision_number for r in p.revisions] or [0]) + 1
        r = HarnessProfileRevision(
            profile_id=p.id,
            revision_number=n,
            manifest=body.manifest.model_dump(),
    ...
    def export_harness_profile(profile_id: str, d: Session = Depends(db_dep)):  # noqa: B008 - FastAPI dependency/body marker
        ...
        if not p:
            raise HTTPException(404, "Harness profile not found")
        r = next(
            (r for r in p.revisions if r.id == p.current_revision_id), p.revisions[-1]
        )
    ...
    def import_harness_profile(body: dict, d: Session = Depends(db_dep)):  # noqa: B008 - FastAPI dependency/body marker
        manifest = (
            ...
            else (body if "command" in body else None)
        )
        if manifest is None:
            raise HTTPException(
                422,
                "Import must contain a raw harness manifest or an exported profile wrapper",
            )
        try:
            parsed = HarnessProfileCreate(
                name=body.get("name", "Imported harness")
        ...
        p = HarnessProfile(name=parsed.name, description=parsed.description)
        d.add(p)
        d.flush()
        r = HarnessProfileRevision(
            profile_id=p.id,
            revision_number=1,
            manifest=parsed.manifest.model_dump(),
            trusted_unsandboxed=False,
        )
        d.add(r)
        d.flush()
    ...
    def harness_probes(profile_id: str, d: Session = Depends(db_dep)):  # noqa: B008 - FastAPI dependency/body marker
        ...
        return [
            ...
            for p in d.query(HarnessProbe)
            .filter(HarnessProbe.revision_id.in_(revision_ids or ["__none__"]))
            .order_by(desc(HarnessProbe.created_at))
            .all()
        ]
    ...
    def probe_harness(
        ...
        if kind not in {"protocol", "full"}:
            raise HTTPException(422, "Probe kind must be protocol or full")
        p = d.get(HarnessProfile, profile_id)
        if not p:
            raise HTTPException(404, "Harness profile not found")
        if p.archived:
        ...
        if not rev:
            raise HTTPException(404, "Harness revision not found")
        import json as _json

        try:
            config = _json.loads(session_config)
        except Exception:
            raise HTTPException(422, "session_config must be JSON") from None
        if not isinstance(config, dict):
            raise HTTPException(422, "session_config must be a JSON object")
        if kind == "full":
            protocol = (
                d.query(HarnessProbe)
                .filter_by(revision_id=rev.id, kind="protocol")
                .order_by(desc(HarnessProbe.created_at))
                .first()
            )
            if not protocol or protocol.status != "verified":
                raise HTTPException(
                    422, "A verified protocol probe is required before a full probe"
                )
        ...
        def execute_probe():
            ...
            try:
                ...
                if kind == "protocol":
                    evidence = asyncio.run(protocol_probe(rev.manifest))
                    row.status = evidence.pop("status", "failed")
                    row.evidence = {**evidence, "manifest_revision_id": rev.id}
                else:
                ...
                if kind == "protocol":
                    ...
                    for frame in frames:
                        ...
                        if not row.evidence.get("config_options") and (
                            "configOptions" in value or "config_options" in value
                        ):
                            row.evidence["config_options"] = value.get(
                                "configOptions"
                            ) or value.get("config_options")
                        if not row.evidence.get("modes") and "modes" in value:
                            row.evidence["modes"] = value["modes"]
                        if not row.evidence.get("agent_info") and isinstance(
                            value.get("agentInfo"), dict
                        ):
                            row.evidence["agent_info"] = value["agentInfo"]
                    row.evidence = dict(row.evidence or {})
                row.agent_identity = (row.evidence or {}).get("agent_identity") or (
                    row.evidence or {}
                ).get("agent_info")
    ...
    def create_profile(body: ProfileCreate, d: Session = Depends(db_dep)):  # noqa: B008 - FastAPI dependency/body marker
        ...
        d.add(p)
        d.flush()
        r = McpProfileRevision(
            profile_id=p.id, revision_number=1, mcp_json=body.mcp_json
        )
        d.add(r)
        d.flush()
        p.current_revision_id = r.id
    ...
    def add_revision(
        ...
        n = max([r.revision_number for r in p.revisions] or [0]) + 1
        r = McpProfileRevision(
            profile_id=p.id, revision_number=n, mcp_json=body.mcp_json
        )
        d.add(r)
        d.flush()
        p.current_revision_id = r.id
    ...
    def run_json(r, d=None):
        ...
        out["failure"] = (
            {"code": "run_failed", "phase": "execution", "message": r.error_message}
            if r.error_message
            else None
        )
        if d is not None and r.harness == "acp":
            snap = d.get(RunHarnessSnapshot, r.id)
            out["harness_snapshot"] = (
                {
                    "revision_id": snap.revision_id,
                    "manifest": snap.manifest,
                    "session_config": snap.session_config,
                    "agent_mode_id": snap.agent_mode_id,
                    "tool_mode": snap.tool_mode,
                    "verification": snap.verification,
                    "observed": (snap.verification or {}).get("observed"),
                }
                if snap
                else None
            )
            if snap:
                ...
                out["effective_model"] = (snap.verification or {}).get(
                    "effective_model"
                ) or "agent-default"
        ...
        end = r.finished_at or now_value
        if start and start.tzinfo is None:
            start = start.replace(tzinfo=timezone.utc)
        if end and end.tzinfo is None:
            end = end.replace(tzinfo=timezone.utc)
        out["elapsed_seconds"] = (
            max(0.0, (end - start).total_seconds()) if start and end else 0.0
        )
        out["queue_position"] = None
        if d is not None and r.status == "queued":
            out["queue_position"] = (
                d.query(Run)
                .filter(Run.status == "queued", Run.created_at < r.created_at)
                .count()
                + 1
            )
        ...
        if d is not None:
            ...
            server = (
                revision.mcp_json.get("mcpServers", {}).get(r.enabled_server)
                if revision
                else None
            ) or {}
    ...
    def create_run(body: RunCreate, d: Session = Depends(db_dep)):  # noqa: B008 - FastAPI dependency/body marker
        ...
        if body.harness == "acp" and not body.harness_revision_id:
            raise HTTPException(422, "harness_revision_id is required")
        if body.harness == "acp" and (
            body.tool_mode != "agent_default" or body.model != "agent-default"
        ):
            raise HTTPException(422, "ACP requires agent-default model and tool mode")
        if body.harness != "acp" and body.tool_mode == "agent_default":
        ...
        if p.archived:
            raise HTTPException(422, "Profile is archived")
        try:
            selected_server_config(rev.mcp_json, body.enabled_server)
        except ProfileValidationError as e:
            raise HTTPException(422, str(e)) from e
        max_turns = settings.claude_max_turns if body.harness == "claude-code" else 0
        max_budget = (
            settings.claude_max_budget_usd if body.harness == "claude-code" else 0
        )
        hrev = (
            d.get(HarnessProfileRevision, body.harness_revision_id)
            if body.harness == "acp"
            else None
        )
        ...
        r = Run(
            profile_revision_id=rev.id,
            enabled_server=body.enabled_server,
    ...
    def runs(
        ...
        if profile_id:
            revision_ids = [
                x[0]
                for x in d.query(McpProfileRevision.id)
                .filter_by(profile_id=profile_id)
                .all()
            ]
        ...
        if harness_profile_id or harness_profile:
            ...
            if harness_kind == "builtin" or harness in {"claude-code", "opencode"}:
                raise HTTPException(
                    422, "harness profile filters apply only to ACP runs"
                )
            revision_ids = [
                x[0]
                for x in d.query(HarnessProfileRevision.id)
                .filter_by(profile_id=harness_profile_id)
                .all()
            ]
            snapshot_run_ids = [
                x[0]
                for x in d.query(RunHarnessSnapshot.run_id)
                .filter(
                    RunHarnessSnapshot.revision_id.in_(revision_ids or ["__none__"])
                )
                .all()
            ]
    ...
    def clone(run_id: str, body: RunClone | None = None, d: Session = Depends(db_dep)):  # noqa: B008 - FastAPI dependency/body marker
        ...
        if o.use_latest_revision:
            ...
            profile = (
                d.get(McpProfile, original_profile.profile_id)
                if original_profile
                else None
            )
        ...
        if profile and profile.archived:
            raise HTTPException(422, "Profile is archived")
        harness = o.harness or old.harness
        model = o.model or old.model
        if model not in settings.models_for(harness):
            raise HTTPException(422, "Model is not configured for this harness")
        if harness == "acp" and (
            model != "agent-default"
            or (o.tool_mode is not None and o.tool_mode != "agent_default")
            or (o.harness is not None and o.harness != "acp")
        ):
            raise HTTPException(422, "ACP requires agent-default model and tool mode")
        ...
        destination_tool_mode = (
            o.tool_mode
            if o.tool_mode is not None
            else (
                "agent_default"
                if harness == "acp"
                else (old.tool_mode if old.tool_mode != "agent_default" else "mcp_only")
            )
        )
        if harness == "acp" and destination_tool_mode != "agent_default":
            raise HTTPException(422, "ACP requires agent-default tool mode")
        if harness != "acp" and destination_tool_mode not in {
            "mcp_only",
            "mcp_read_only",
            "full",
        }:
            raise HTTPException(422, "Invalid tool mode for destination harness")
        prompt = o.prompt if o.prompt is not None else old.prompt
        expected = (
            o.expected_output if o.expected_output is not None else old.expected_output
        )
        if not prompt.strip() or not expected.strip():
            raise HTTPException(422, "Prompt and expected output must be nonblank")
        ...
        r = Run(
            ...
            profile_revision_id=rev.id,
            enabled_server=o.enabled_server or old.enabled_server,
            harness=harness,
            model=model,
            tool_mode=destination_tool_mode,
            prompt=prompt,
        ...
        try:
            selected_server_config(rev.mcp_json, r.enabled_server)
        except ProfileValidationError as e:
            raise HTTPException(422, str(e)) from e
        d.add(r)
        d.flush()
        if harness == "acp":
            ...
            if (
                not old_snapshot
                and not o.harness_revision_id
                and not o.use_latest_harness_revision
            ):
                raise HTTPException(
                    422, "ACP harness snapshot unavailable or untrusted"
                )
            selected_harness_revision = (
                o.harness_revision_id
                if o.harness_revision_id
                else (old_snapshot.revision_id if old_snapshot else None)
            )
            ...
            selected = (
                d.get(HarnessProfileRevision, selected_harness_revision)
                if selected_harness_revision
                else None
            )
            if (
                not selected
                or not selected.trusted_unsandboxed
                or (selected.profile and selected.profile.archived)
            ):
                raise HTTPException(
                    422, "ACP harness snapshot unavailable or untrusted"
                )
            ...
            unchanged_snapshot = bool(
                old_snapshot
                and o.harness is None
                and o.harness_revision_id is None
                and o.profile_revision_id is None
                and o.enabled_server is None
                and not o.use_latest_harness_revision
                and not o.use_latest_revision
                and o.agent_mode_id is None
                and o.session_config is None
                and o.transport is None
                and o.tool_mode is None
            )
            if not unchanged_snapshot:
                next_config = (
                    o.session_config
                    if o.session_config is not None
                    else (old_snapshot.session_config if old_snapshot else {})
                )
                next_mode = (
                    o.agent_mode_id
                    if o.agent_mode_id is not None
            ...
            if unchanged_snapshot:
                ...
                snapshot_kwargs = {
                    "revision_id": old_snapshot.revision_id,
                    "manifest": copy.deepcopy(old_snapshot.manifest),
                    "session_config": copy.deepcopy(old_snapshot.session_config),
                    "agent_mode_id": old_snapshot.agent_mode_id,
                    "tool_mode": old_snapshot.tool_mode,
                    "verification": copy.deepcopy(old_snapshot.verification),
                }
            else:
                session_config = (
                    o.session_config
                ...
                configured_transport = transport_for_server(
                    rev.mcp_json.get("mcpServers", {}).get(r.enabled_server) or {}
                )
                if o.transport is not None and o.transport != configured_transport:
                    raise HTTPException(
                        422, "transport override does not match selected MCP server"
                    )
                snapshot_kwargs = {
                    ...
                    "verification": _verification_for(
                        d,
                        selected,
                        transport=o.transport or configured_transport,
                        mode_id=mode_id,
    ...
    def delete_run(run_id: str, d: Session = Depends(db_dep)):  # noqa: B008 - FastAPI dependency/body marker
        ...
        if r.status in ("queued", "running"):
            raise HTTPException(
                409, "Active runs cannot be deleted; cancel and wait first"
            )
        d.query(RunEvent).filter_by(run_id=run_id).delete(synchronize_session=False)
        d.query(RunTrace).filter_by(run_id=run_id).delete(synchronize_session=False)
        d.query(RunHarnessSnapshot).filter_by(run_id=run_id).delete(
            synchronize_session=False
        )
        d.delete(r)
        d.commit()
    ...
    def clear_runs(
        ...
        if active:
            raise HTTPException(
                409,
                "Active runs must be cancelled and finished before clearing history",
            )
        for r in d.query(Run).all():
            d.query(RunEvent).filter_by(run_id=r.id).delete(synchronize_session=False)
            d.query(RunTrace).filter_by(run_id=r.id).delete(synchronize_session=False)
            d.query(RunHarnessSnapshot).filter_by(run_id=r.id).delete(
                synchronize_session=False
            )
            d.delete(r)
        d.commit()
    ...
    def report(run_id: str, d: Session = Depends(db_dep)):  # noqa: B008 - FastAPI dependency/body marker
        ...
        transport = transport_for_server(
            (rev.mcp_json.get("mcpServers", {}).get(r.enabled_server) if rev else None)
            or {}
        )
        mcp_summary = derive_mcp_summary([e.raw_event for e in ev], r.enabled_server)
        mcp_summary["transport"] = transport
        ...
        trace_schema = (
            stored_trace.schema_version
            if stored_trace
            else ("claude.v1" if r.harness == "claude-code" else None)
        )
        unavailable_reason = (
            "Trace unavailable for legacy run."
            if r.harness == "claude-code"
            else f"Trace parsing is not implemented for {r.harness}."
        )
        report_value = {
            "run": run_json(r, d),
            "profile_revision": {
                ...
                "mcp_json": safe_profile,
            },
            "assertions": {
                "lifecycle": r.status,
                "mcp": {"status": r.mcp_assertion},
                "semantic": {
                    "status": r.semantic_assertion,
            ...
            "high_risk": r.tool_mode == "full",
            "warning": "Downloaded reports and persisted trace payloads redact detected credentials.",
            "trace": {
                "available": stored_trace is not None,
                "schema": trace_schema,
                "harness": (trace_value or {}).get("harness", r.harness),
                "capture_status": stored_trace.capture_status
                if stored_trace
                else "unavailable",
                "summary": (trace_value or {}).get("summary", {"transport": transport}),
                "mcp_calls_schema": (trace_value or {}).get("mcp_calls_schema")
                if trace_value
                else None,
                "mcp_calls": (trace_value or {}).get("mcp_calls", []),
                "spans": (trace_value or {}).get("spans", []),
                "protocol_events": (trace_value or {}).get("protocol_events", []),
                "acp_protocol_events": (trace_value or {}).get(
                    "acp_protocol_events",
                    (trace_value or {}).get("protocol_events", []),
                ),
                "mcp_protocol_events": (trace_value or {}).get(
                    "mcp_protocol_events", []
                ),
                "result_metadata": (trace_value or {}).get("result_metadata", {}),
                "limitations": (trace_value or {}).get(
