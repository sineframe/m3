...
class OpenCodeRunner(HarnessRunner):
    def __init__(self, executable: str = "opencode", api_key: str | None = None, provider_api_keys: dict[str, str] | None = None):
        self.executable = executable; self.api_key = api_key; self.provider_api_keys = dict(provider_api_keys or {}); self.process: Any = None; self.cancel_requested = threading.Event()
        ...
        with tempfile.TemporaryDirectory(prefix="mcp-pal-opencode-") as td:
            ...
            try:
                if result.transport == "stdio":
                    ...
                    secrets.update(write_stdio_handoff(handoff, configured_server.get("env") or {}))
                    server["environment"] = {}
                    relay_command = [
                        sys.executable, "-m", "mcp_pal.transport.stdio_proxy",
                        "--capture", capture_path, "--baseline", str(baseline),
                        "--env-file", handoff,
                    ]
                    if isinstance(configured_server.get("cwd"), str) and configured_server["cwd"]:
                    ...
                    proxy = McpHttpProxy(upstream_url=configured_server["url"], configured_headers=configured_server.get("headers"), transport=result.transport, capture_path=capture_path, baseline_ns=baseline, secrets=secrets)
            ...
            try:
                ...
                for provider,key in self.provider_api_keys.items():
                    ...
                    variable="OPENCODE_API_KEY" if provider.lower() in ("opencode", "opencode-go") else f"{normalized_provider}_API_KEY"
            ...
            try:
                ...
                stdout_task=asyncio.create_task(read_stdout()); stderr_task=asyncio.create_task(drain_bounded(self.process.stderr, maximum=_STDERR_LIMIT)); wait_task=asyncio.create_task(self.process.wait()); cancel_task=asyncio.create_task(cancel_event.wait()) if cancel_event else None
                try:
                    ...
                    while not wait_task.done():
                        ...
                        if result.session_id and time.monotonic() >= next_recovery:
                            ...
                            if exported:
                                ...
                                result.turns=len(finishes); costs=[float(p["cost_usd"]) for p in finishes if isinstance(p.get("cost_usd"),(int,float))]; result.cost_usd=sum(costs) if costs else result.cost_usd
                ...
                result.stderr=_redact((await stderr_task).decode(errors="replace"), secrets); result.exit_code=self.process.returncode
            ...
            except FileNotFoundError as error: result.status,result.error="failed",_redact(str(error), secrets)
