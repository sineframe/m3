...
def _event_key(event_type: str, payload: dict) -> tuple:
    return (
        ...
        payload.get("text"),
        json.dumps(payload.get("step"), sort_keys=True, default=str)
        if payload.get("step")
        else None,
    )
    ...
    for index, item in enumerate(normalized):
        ...
        if key in known:
            ...
            if (
                ...
                and previous
                and previous[-1][0] == "assistant_text"
            ):
                fresh.append(item)
            continue
        fresh.append(item)
...
def _env_refs(value: Any) -> Any:
    ...
    if isinstance(value, list):
        return [_env_refs(item) for item in value]
    if isinstance(value, dict):
        return {key: _env_refs(item) for key, item in value.items()}
    return value
...
def opencode_config(mcp_config: dict, server_name: str, tool_mode: str) -> dict:
    ...
    if source.get("type", "stdio") == "stdio":
        server = {
            "type": "local",
            "command": [source["command"], *source.get("args", [])],
            "enabled": True,
        }
        if source.get("env"):
            server["environment"] = _env_refs(source["env"])
    else:
        server = {"type": "remote", "url": source["url"], "enabled": True}
        if source.get("headers"):
            server["headers"] = _env_refs(source["headers"])
...
class OpenCodeRunner(HarnessRunner):
    ...
    async def _export_completed_session(
        ...
        try:
            ...
            try:
                stdout, _ = await asyncio.wait_for(
                    asyncio.shield(communicate), timeout=5
                )
        ...
        if not last_parts or last_parts[-1].get("type") != "step-finish":
            return None
        if not any(
        ...
        for message in assistant:
            for part in message.get("parts", []):
                ...
                if typ and (
                    typ != "tool_use"
                    or part.get("state", {}).get("status") in ("completed", "error")
                ):
                    recovered.append(
                        {
                            "type": typ,
                            "timestamp": part.get("time", {}).get("end")
                            or part.get("time", {}).get("start"),
    ...
    async def run(
        ...
        secrets.update(value for value in self.provider_api_keys.values() if value)
        with tempfile.TemporaryDirectory(prefix="mcp-pal-opencode-") as td:
            ...
            configured_server = selected_server_config(
                spec.mcp_config, spec.enabled_server
            )["mcpServers"][spec.enabled_server]
            result.transport = transport_for_server(configured_server)
            native_config = opencode_config(
                spec.mcp_config, spec.enabled_server, spec.tool_mode
            )
            ...
            try:
                if result.transport == "stdio":
                    ...
                    secrets.update(
                        write_stdio_handoff(handoff, configured_server.get("env") or {})
                    )
                    server["environment"] = {}
                    relay_command = [
                        sys.executable,
                    ...
                    if (
                        isinstance(configured_server.get("cwd"), str)
                        and configured_server["cwd"]
                    ):
                        relay_command.extend(["--cwd", configured_server["cwd"]])
                    relay_command.extend(["--", *command])
                    server["command"] = relay_command
                elif result.transport in {"http", "sse"}:
                    proxy = McpHttpProxy(
                        upstream_url=configured_server["url"],
                        configured_headers=configured_server.get("headers"),
                        transport=result.transport,
                        capture_path=capture_path,
                        baseline_ns=baseline,
                        secrets=secrets,
                    )
                    native_config["mcp"][spec.enabled_server][
                        "url"
                    ] = await proxy.start()
                ...
                result.status, result.error = (
                    "failed",
                    "OpenCode transport setup failed",
                )
                try:
            ...
            try:
                Path(config_path).write_text(
                    json.dumps(native_config), encoding="utf-8"
                )
            ...
            try:
                ...
                environment["OPENCODE_CONFIG"] = config_path
                environment["PWD"] = td
                ...
                environment["HOME"] = td
                original_data = environment.get("XDG_DATA_HOME") or os.path.join(
                    original_home, ".local", "share"
                )
                ...
                if os.path.isfile(auth_source):
                    try:
                        os.makedirs(
                            os.path.dirname(auth_target), mode=0o700, exist_ok=True
                        )
                        ...
                        pass
                if self.api_key:
                    environment["OPENCODE_API_KEY"] = self.api_key
                for provider, key in self.provider_api_keys.items():
                    normalized_provider = provider.lower().replace("-", "_").upper()
                    variable = (
                        "OPENCODE_API_KEY"
                        if provider.lower() in ("opencode", "opencode-go")
                        else f"{normalized_provider}_API_KEY"
                    )
            ...
            try:
                    ...
                    async for line in self.process.stdout:
                        ...
                        result.event_records.append(
                            {
                                ...
                                "offset_ms": max(
                                    0.0, (time.perf_counter_ns() - baseline) / 1_000_000
                                ),
                                "raw_event": raw,
                                "type": raw.get("type")
                        ...
                        for event_type, payload in normalized:
                            if event_type == "assistant_text":
                                result.final_text += str(payload.get("text") or "")
                ...
                try:
                    ...
                    next_recovery = time.monotonic() + 3
                    while not wait_task.done():
                        ...
                        if result.session_id and time.monotonic() >= next_recovery:
                            exported = await self._export_completed_session(
                                result.session_id, environment, td
                            )
                            next_recovery = time.monotonic() + 2
                            if exported:
                                known = {
                                    _event_key(event_type, payload)
                                    for event_type, payload in result.normalized
                                }
                                for raw in exported:
                                    raw = _redact(raw, secrets)
                                    normalized = normalize_events(
                                        raw, spec.enabled_server
                                    )
                                    fresh = _fresh_recovery(
                                        normalized, known, result.normalized
                                    )
                                    if not fresh:
                                        continue
                                    result.events.append(raw)
                                    result.event_records.append(
                                        {
                                            "source": "opencode-recovery",
                                            "offset_ms": max(
                                                0.0,
                                                (time.perf_counter_ns() - baseline)
                                                / 1_000_000,
                                            ),
                                            "raw_event": raw,
                                            "type": raw.get("type")
                                    ...
                                    result.normalized.extend(fresh)
                                ...
                                result.cost_usd = (
                                    sum(costs) if costs else result.cost_usd
                                )
                ...
                # Ensure descendants cannot retain the stderr pipe after the
                # direct process has exited normally.
                await self._terminate_and_reap()
                            ...
                            result.event_records.append(
                                {
                                    ...
                                    "offset_ms": max(
                                        0.0,
                                        (time.perf_counter_ns() - baseline) / 1_000_000,
                                    ),
                                    "raw_event": raw,
                                    "type": raw.get("type")
                                    if isinstance(raw, dict)
                ...
                if result.status == "running":
                    result.status = (
                        "completed"
                        if result.exit_code == 0 and complete_stream
                        else "failed"
                    )
                    if (
                        ...
                        result.error = (
                            "OpenCode exited before emitting a complete JSON trace"
                        )
                if result.status != "completed" and result.exit_code not in (0, None):
                    result.error = f"OpenCode exited with code {result.exit_code}"
            except FileNotFoundError as error:
                result.status, result.error = "failed", _redact(str(error), secrets)
            except asyncio.CancelledError:
                raise
