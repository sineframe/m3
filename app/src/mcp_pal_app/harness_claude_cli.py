...
class ClaudeCodeRunner(HarnessRunner):
    def __init__(self, executable: str = "claude"):
        self.executable = executable; self.process: Any = None; self.cancel_requested = threading.Event()
        ...
        with tempfile.TemporaryDirectory(prefix="mcp-pal-") as td:
            ...
            try:
                if result.transport == "stdio":
                    ...
                    secrets.update(write_stdio_handoff(handoff, original.get("env") or {}))
                    ...
                    original["env"] = {}
                    relay_args = [
                        "-m", "mcp_pal.transport.stdio_proxy", "--capture", capture_path,
                        "--baseline", str(baseline_ns), "--env-file", handoff,
                    ]
                    ...
                    proxy = McpHttpProxy(upstream_url=server["url"], configured_headers=server.get("headers"), transport=result.transport, capture_path=capture_path, baseline_ns=baseline_ns, secrets=secrets)
            ...
            try:
                ...
                stderr_task = asyncio.create_task(drain_bounded(self.process.stderr, maximum=_STDERR_LIMIT))
                ...
                result.stderr = _redact((await stderr_task).decode(errors="replace"), secrets)
                result.exit_code = self.process.returncode
            ...
            except FileNotFoundError as e: result.status, result.error = "failed", _redact(str(e), secrets)
