...
from mcp_pal.harness.base import RunSpec, AcpRunSpec
...
from mcp_pal_app.persistence.models import McpProfileRevision, HarnessProfileRevision, RunHarnessSnapshot, Run, RunEvent, RunTrace, now
...
from mcp_pal.trace.redaction import RedactionConfig, redact_for_log, redact_for_persistence
...
def _observed_acp_metadata(result):
    ...
    observed={"session_id":getattr(result, "session_id", None),"agent_identity":None,"mode_id":None,"session_config":{},"modes":None,"config_options":None,"configured_transport":getattr(result, "configured_transport", None) or getattr(result, "transport", None),"instrumented_transport":getattr(result, "instrumented_transport", None) or getattr(result, "transport", None)}
    for frame in frames:
        ...
        if isinstance(value.get("agentInfo"),dict): observed["agent_identity"]=value["agentInfo"]
        if value.get("sessionId") or value.get("session_id"):
            observed["session_id"]=value.get("sessionId") or value.get("session_id")
        if "modes" in value: observed["modes"]=value["modes"]
        if "configOptions" in value or "config_options" in value:
            observed["config_options"]=value.get("configOptions") or value.get("config_options")
        method=payload.get("method")
        params=payload.get("params") if isinstance(payload.get("params"),dict) else {}
        if method == "session/set_mode": observed["mode_id"]=params.get("modeId") or params.get("mode_id")
        elif method == "session/set_config_option":
            ident=params.get("configId") or params.get("config_id")
            if ident is not None: observed["session_config"][ident]=params.get("value")
    if observed["mode_id"] is None and isinstance(observed.get("modes"),dict):
        observed["mode_id"]=observed["modes"].get("currentModeId") or observed["modes"].get("current_mode_id")
    return observed
...
class RunManager:
    ...
    def runner_for(self, harness):
        ...
        if harness == "acp": raise ValueError("ACP runner requires a revision manifest")
        raise ValueError(f"Unsupported harness: {harness}")
    def execute(self, run_id):
        db=self.session_factory(); run=db.get(Run,run_id)
        if not run or run.status != "queued": db.close(); return
        run.status,run.started_at="running",now(); db.commit(); rev=db.get(McpProfileRevision,run.profile_revision_id); cancel=asyncio.Event(); snapshot=db.get(RunHarnessSnapshot,run.id) if run.harness == "acp" else None; runner=(AcpHarnessRunner(snapshot.manifest) if snapshot else None) if run.harness == "acp" else self.runner_for(run.harness); done=threading.Event()
        ...
        async def callback(raw,event_type,payload):
            nonlocal seq
            seq+=1
            safe_payload = redact_for_persistence(payload, config=config, path="$.event.payload")
            safe_raw = redact_for_persistence(raw, config=config, path="$.event.raw_event")
            db.add(RunEvent(run_id=run_id,sequence=seq,event_type=event_type,payload=safe_payload,raw_event=safe_raw)); db.commit()
        result = None
        try:
            if run.harness == "acp":
                if not snapshot: raise ValueError("ACP harness snapshot not found")
                spec=AcpRunSpec(run.prompt,run.model,rev.mcp_json,run.enabled_server,snapshot.manifest,snapshot.tool_mode,snapshot.agent_mode_id,snapshot.session_config,run.timeout_seconds)
            else:
            ...
            safe_result = redact_for_persistence(result.final_text, config=config, path="$.run.final_output")
            safe_stderr = redact_for_log(result.stderr, config=config, path="$.run.stderr")
            safe_error = redact_for_log(result.error, config=config, path="$.run.error")
            run.status,run.claude_result,run.exit_code,run.error_message,run.stderr=result.status,safe_result,result.exit_code,safe_error,safe_stderr; run.cost_usd,run.turns,run.session_id=result.cost_usd,result.turns,result.session_id; run.mcp_assertion=derive_mcp_assertion(result.events,run.enabled_server); run.finished_at=now()
            if run.harness == "acp" and (result.error or result.error_code or result.error_details):
                snapshot=db.get(RunHarnessSnapshot,run.id)
                if snapshot:
                    safe_details = redact_for_persistence(result.error_details or {}, config=config, path="$.run.failure.diagnostics")
                    verification=dict(snapshot.verification or {})
                    verification["failure"]={"code":result.error_code or "acp_run_failed","phase":result.error_phase or "execution","message":safe_error,"diagnostics":safe_details}
                    snapshot.verification=verification
            if run.harness == "acp":
                snapshot=db.get(RunHarnessSnapshot,run.id)
                if snapshot:
                    observed=_observed_acp_metadata(result)
                    safe_observed = redact_for_persistence(observed, config=config, path="$.run.observed")
                    verification=dict(snapshot.verification or {})
            ...
            if run.harness == "acp":
                ...
                acp_transport = getattr(result, "configured_transport", None) or getattr(result, "transport", None) or "unknown"
                instrumented_transport = getattr(result, "instrumented_transport", None) or acp_transport
                trace=build_acp_trace(acp_frames=result.event_records or (),mcp_frames=result.protocol_events or (),selected_server=run.enabled_server,configured_transport=acp_transport,instrumented_transport=instrumented_transport,status=result.status,session_id=result.session_id,result_metadata=(db.get(RunHarnessSnapshot,run.id).verification if db.get(RunHarnessSnapshot,run.id) else {}))
                ...
                reported_calls=[c for c in calls if c.get("provenance",{}).get("model")]
                wire_calls=[c for c in calls if c.get("provenance",{}).get("wire")]
                unproven_reported=[c for c in reported_calls if not c.get("provenance",{}).get("wire")]
                run.mcp_assertion="passed" if wire_calls and not unproven_reported and all(c.get("status")=="completed" and c.get("wire_request") is not None and c.get("wire_response") is not None for c in wire_calls) else "failed"
            if run.harness == "claude-code":
                ...
                safe_trace = redact_for_persistence(trace, config=config, path="$.trace")
            ...
            exception_message=redact_for_log(str(e), config=config, path="$.run.error")
            run.status,run.error_message,run.finished_at="failed",exception_message,now()
            if run.harness == "acp" and result is not None:
                snapshot=db.get(RunHarnessSnapshot,run.id)
                if snapshot and (result.error or result.error_code or result.error_details):
                    safe_error = redact_for_log(result.error, config=config, path="$.run.failure.message")
                    safe_details = redact_for_persistence(result.error_details or {}, config=config, path="$.run.failure.diagnostics")
                    verification=dict(snapshot.verification or {})
                    verification["failure"]={"code":result.error_code or "acp_run_failed","phase":result.error_phase or "execution","message":safe_error or str(e),"diagnostics":safe_details}
                    snapshot.verification=verification
            ...
            if result is not None and run.harness == "claude-code":
                ...
                safe_trace=redact_for_persistence(trace, config=config, path="$.trace"); db.merge(RunTrace(run_id=run.id,harness="acp",schema_version=trace["schema"],capture_status="partial",trace=safe_trace))
            db.commit()
        finally:
    ...
    def shutdown(self):
        with self.lock:
            for runner in self.runners.values():
                if runner is not None: runner.request_cancel()
