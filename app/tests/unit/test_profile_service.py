...
def _mcp(service: ProfileService) -> str:
    profile_id = service.create_mcp(
        MCPProfileInput(
            ...
            config={
                "mcpServers": {
                    "stdio": {"command": "echo", "args": ["ok"], "env": {"TOKEN": "${MCP_TOKEN}"}, "cwd": "/tmp/work"},
                    "http": {"type": "http", "url": "https://example.test/mcp", "headers": {"Authorization": "${MCP_AUTH}"}},
...
def test_profile_lifecycle_import_export_and_trust(tmp_path: Path) -> None:
    ...
    profile = service.create_mcp(MCPProfileInput(name="one", config={"mcpServers": {"echo": {"command": "echo"}}}))
    assert profile.current_revision is not None
    revised = service.add_mcp_revision(profile.record.id, {"mcpServers": {"echo": {"command": "printf"}}})
    assert revised.current_revision is not None and revised.current_revision.revision_number == 2
    assert service.update_mcp(profile.record.id, description="").record.description == ""
    ...
    harness = service.create_harness(HarnessProfileInput(name="agent", manifest={"command": "agent"}, trusted_unsandboxed=True))
    assert harness.current_revision is not None
    assert '"manifest"' in service.export_harness(harness.record.id)
    imported = service.import_harness({"name": "copy", "manifest": {"command": "agent"}, "trusted_unsandboxed": True})
    ...
    with pytest.raises(ProfileServiceError, match="acknowledgment"):
        service.add_harness_revision(harness.record.id, {"command": "agent"}, trusted_unsandboxed=False)
    store.close()
...
def test_spec_builder_converts_all_transports_and_secret_references(tmp_path: Path, server: str, expected: type) -> None:
    ...
    settings = Settings(claude_model_ids=["claude-test"], opencode_model_ids=["openrouter/test"])
    spec = ExecutionSpecBuilder(store, settings).build(
        OneTurnRunDraft(profile_id=profile_id, enabled_server=server, model="claude-test", prompt="do it", expected_goal="it is done")
    )
    value = spec.servers[0].server
    assert isinstance(value, expected)
    assert spec.message is not None and isinstance(spec.message.content[0], TextContent) and spec.message.content[0].text == "do it"
...
def test_spec_builder_harness_and_policy_mapping(tmp_path: Path, harness: str) -> None:
    ...
    for mode in ("mcp_only", "mcp_read_only", "full"):
        spec = ExecutionSpecBuilder(store, settings).build(OneTurnRunDraft(profile_id=profile_id, enabled_server="stdio", harness=harness, model=model, prompt="p", expected_goal="g", tool_mode=mode))
        assert isinstance(spec.harness, OpenCode if harness == "opencode" else ClaudeCode)
        assert isinstance(spec.tool_policy, NativeToolPolicy)
        assert spec.tool_policy.policy["mode"] == mode
        if mode == "mcp_read_only":
            expected_tools = ("Agent", "Read", "Glob", "Grep", "LSP", "WebFetch", "WebSearch", "ToolSearch", "ListMcpResourcesTool", "ReadMcpResourceTool", "TaskGet", "TaskList", "TaskOutput") if harness == "claude-code" else ("read", "glob", "grep", "lsp", "webfetch", "websearch")
            assert spec.tool_policy.policy["read_only_tools"] == expected_tools
    opencode = ExecutionSpecBuilder(store, settings).build(OneTurnRunDraft(profile_id=profile_id, enabled_server="stdio", harness="opencode", model="openrouter/test", prompt="p", expected_goal="g"))
...
def test_spec_builder_acp_options_and_invalid_selection(tmp_path: Path) -> None:
    store, service = _service(tmp_path)
    profile_id = _mcp(service)
    harness = service.create_harness(HarnessProfileInput(name="acp", manifest={"command": "agent", "args": ["--stdio"], "env": {"KEY": "${AGENT_KEY}"}}, trusted_unsandboxed=True))
    settings = Settings()
    spec = ExecutionSpecBuilder(store, settings).build(OneTurnRunDraft(profile_id=profile_id, enabled_server="stdio", harness="acp", harness_profile_id=harness.record.id, model="agent-default", tool_mode="agent_default", prompt="p", expected_goal="g", agent_mode_id="fast", session_config={"engine": "x"}))
    ...
    with pytest.raises(ProfileServiceError, match="selected MCP server"):
        ExecutionSpecBuilder(store, settings).build(OneTurnRunDraft(profile_id=profile_id, enabled_server="missing", model="agent-default", prompt="p", expected_goal="g"))
    service.archive_mcp(profile_id)
    with pytest.raises(ProfileServiceError, match="archived"):
        ExecutionSpecBuilder(store, settings).build(OneTurnRunDraft(profile_id=profile_id, enabled_server="stdio", model="agent-default", prompt="p", expected_goal="g"))
    store.close()


def test_typed_credentials_and_acp_options_round_trip_through_durable_queue(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    canary = "claude-credential-canary"
    monkeypatch.setenv("ANTHROPIC_API_KEY", canary)
    store, service = _service(tmp_path)
    profile_id = _mcp(service)
    harness = service.create_harness(HarnessProfileInput(name="acp", manifest={"command": "agent"}, trusted_unsandboxed=True))
    settings = Settings(claude_model_ids=["claude-test"])
    builder = ExecutionSpecBuilder(store, settings)
    specs = (
        builder.build(OneTurnRunDraft(profile_id=profile_id, enabled_server="stdio", model="claude-test", prompt="p", expected_goal="g")),
        builder.build(OneTurnRunDraft(profile_id=profile_id, enabled_server="stdio", harness="acp", harness_profile_id=harness.record.id, model="agent-default", tool_mode="agent_default", prompt="p", expected_goal="g", agent_mode_id="fast", session_config={"quality": "high"})),
    )
