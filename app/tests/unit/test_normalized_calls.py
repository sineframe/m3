...
def _span(name, ident, start, output=None, status="completed", meta=None):
    return {
        ...
        "end_ms": start + 3,
        "duration_ms": 3,
        "input": {"name": "argument-value", "x": ident},
        "output": output,
        "status": status,
        "metadata": {"mcp_selected": meta if meta is not None else True},
    }
...
def test_claude_excludes_builtin_and_preserves_argument_name_and_error():
    trace = {
        "spans": [
            _span("Read", "builtin", 1, "file", meta=False),
            _span("mcp__draw__paint", "m1", 2, {"error": "bad"}, "error"),
        ],
        "mcp_calls": [],
    }
    ...
    assert (
        ...
        and calls[0]["arguments"]["name"] == "argument-value"
    )
    assert (
        calls[0]["error"] == "bad"
        and calls[0]["status"] == "error"
        and calls[0]["duration_ms"] == 3
    )


def test_claude_repeated_same_name_pairs_wire_in_order_and_exact_arguments_result():
    spans = [
        _span("mcp__draw__paint", "m1", 1, "one"),
        _span("mcp__draw__paint", "m2", 10, "two"),
    ]
    spans += [
        {
            "id": "w1",
            "kind": "mcp",
            "name": "tools/call",
            "input": {"params": {"name": "paint", "arguments": {"a": 1}}},
            "output": {"result": {"n": 1}},
            "duration_ms": 2,
        },
        {
            ...
            "kind": "mcp",
            "name": "tools/call",
            "input": {"params": {"name": "paint", "arguments": {"a": 2}}},
            "output": {"result": {"n": 2}},
            "duration_ms": 4,
        },
    ]
    ...
    assert [c["wire_request"]["params"]["arguments"] for c in calls] == [
        {"a": 1},
        {"a": 2},
    ]
    assert [c["wire_response"] for c in calls] == [
        {"result": {"n": 1}},
        {"result": {"n": 2}},
    ]
...
def test_opencode_wrappers_recovery_duplicates_and_unknown_timing():
    events = [
        {
            ...
            "raw_event": {
                ...
                "part": {
                    "tool": "draw_paint",
                    "callID": "c1",
                    "state": {"input": {"x": 1}, "status": "running"},
                },
            },
        },
        {
    ...
    assert (
        len(calls) == 2
        and calls[0]["tool"] == "paint"
        and calls[0]["result"] == {"ok": True}
        and calls[0]["duration_ms"] == 30
    )
    assert calls[0]["start_ms"] == 8 and calls[0]["end_ms"] == 38
    assert (
        calls[1]["status"] == "error"
        and calls[1]["error"] == "nope"
...
def test_opencode_wire_frames_correlate_repeated_calls_and_errors():
    ...
    protocol = [
        {
            ...
            "payload": {
                "id": 1,
                "method": "tools/call",
                "params": {"name": "echo", "arguments": {"name": "inside"}},
            },
        },
        {
            "offset_ms": 3,
            "direction": "server_to_client",
            "payload": {"id": 1, "result": "one"},
        },
        {
            "offset_ms": 4,
            "direction": "client_to_server",
            "payload": {
                "id": 2,
                "method": "tools/call",
                "params": {"name": "echo", "arguments": {"name": "inside2"}},
            },
        },
        {
            "offset_ms": 6,
            "direction": "server_to_client",
            "payload": {"id": 2, "error": {"message": "bad"}},
        },
    ]
    trace = build_opencode_trace(
        events=events,
    ...
    assert [call["arguments"] for call in trace["mcp_calls"]] == [
        {"name": "inside"},
        {"name": "inside2"},
    ]
    assert trace["mcp_calls"][0]["wire_request"]["params"]["arguments"] == {
        "name": "inside"
    }
    assert (
        trace["mcp_calls"][0]["provenance"]["wire"] is True
        and trace["mcp_calls"][0]["server_latency_ms"] == 2
    )
    assert trace["mcp_calls"][0]["limitations"] == [] and trace["limitations"] == []
    assert trace["mcp_calls"][1]["status"] == "error" and trace["mcp_calls"][1][
        "error"
    ] == {"message": "bad"}
    ...
    assert [span["parent_id"] for span in wire_spans] == [
        span["id"] for span in tool_spans
    ]
...
def test_opencode_waterfall_preserves_steps_reasoning_text_and_summary():
    events = [
        {
            "offset_ms": 1,
            "raw_event": {"type": "step_start", "part": {"type": "step-start"}},
        },
        {
            "offset_ms": 2,
            "raw_event": {
        ...
        {
            "offset_ms": 9,
            "raw_event": {"type": "text", "part": {"type": "text", "text": "done"}},
        },
        {
            "offset_ms": 10,
            "raw_event": {
    ...
    protocol = [
        {
            ...
            "payload": {
                "id": 1,
                "method": "tools/call",
                "params": {"name": "echo", "arguments": {"text": "actual"}},
            },
        },
        {
            "offset_ms": 6,
    ...
    assert len(turns) == 2
    assert [[step["kind"] for step in turn["steps"]] for turn in turns] == [
        ["thinking", "tool_call"],
        ["text"],
    ]
    ...
    assert trace["summary"] == {
        ...
        "total_tokens": 10,
        "cost_usd": 0.03,
        "thinking": {"state": "visible", "count": 1},
    }
    ...
    assert (
        tool["input"] == {"text": "actual"}
        and wire["parent_id"] == tool["id"]
        and wire["duration_ms"] == 2.5
    )
...
def test_opencode_adjacent_reasoning_chunks_are_combined_under_model_turn():
    events = [
        ...
        },
        {
            ...
            "raw_event": {
                "type": "reasoning",
                "part": {"type": "reasoning", "text": "first "},
            },
        },
        {
            "offset_ms": 3,
            "raw_event": {
                "type": "reasoning",
                "part": {"type": "reasoning", "text": "second"},
            },
        },
        {
            "offset_ms": 4,
            "raw_event": {"type": "text", "part": {"type": "text", "text": "answer"}},
        },
        {
            "offset_ms": 5,
            "raw_event": {"type": "step_finish", "part": {"type": "step-finish"}},
        },
    ]
    trace = build_opencode_trace(
        events=events, selected_server="draw", transport="stdio", status="completed"
    )
...
def test_opencode_unmatched_call_has_precise_limitation_only():
    ...
    assert (
        call["wire_request"] is None
        and "correlated" in call["limitations"][0]
        and "interception" not in call["limitations"][0]
    )
...
def test_opencode_protocol_metadata_is_sorted_and_stable():
    ...
    assert (
        [x["sequence"] for x in trace["protocol_events"]] == [1, 2]
        and trace["protocol_events"][0]["method"] == "initialize"
        and trace["protocol_events"][1]["status"] == "response"
    )


def test_opencode_empty_trace_has_no_false_limitation():
    trace = build_opencode_trace(
        events=[], selected_server="draw", transport="stdio", status="completed"
    )
...
def test_opencode_running_and_pending_are_preserved():
    events = [
        {
            "type": "tool_use",
            "part": {"tool": "draw_a", "callID": "r", "state": {"status": "running"}},
        },
        {
            "type": "tool_use",
            "part": {"tool": "draw_b", "callID": "p", "state": {"status": "pending"}},
        },
    ]
    assert [c["status"] for c in from_opencode_events(events, "draw")] == [
        "running",
        "pending",
    ]
...
def test_opencode_native_duration_uses_elapsed_start_end():
    event = {
        ...
        "part": {
            ...
            "time": {"start": 1000000, "end": 1000042},
        },
    }
...
def test_opencode_wire_error_marks_call_error():
    event = {
        ...
        "part": {
            "tool": "draw_a",
            "callID": "e",
            "state": {"status": "completed", "output": "bad"},
        },
    }
    protocol = [
        {
        ...
        },
        {
            "offset_ms": 2,
            "direction": "server_to_client",
            "payload": {"id": 1, "error": {"code": -1}},
        },
    ]
    call = build_opencode_trace(
        events=[event],
        protocol_events=protocol,
        selected_server="draw",
        transport="stdio",
