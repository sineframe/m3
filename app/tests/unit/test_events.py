...
def call(ok=True):
    return {
        "type": "user",
        "tool_name": "mcp__draw__create",
        "tool_use_result": ("ok" if ok else {"is_error": True, "error": "bad"}),
    }
...
def use():
    return {
        "type": "assistant",
        "message": {"content": [{"type": "tool_use", "name": "mcp__draw__create"}]},
    }
            ...
            [
                {
                    "type": "assistant",
                    "message": {
                        "content": [{"type": "tool_use", "name": "mcp__other__x"}]
                    },
                },
                call(),
            ],
...
def test_initialization_summary_requires_selected_status():
    ...
    assert (
        derive_mcp_summary([generic], "draw")["initialization_state"] == "init_observed"
    )
    connected = {
        "type": "system",
        "subtype": "init",
        "mcp_servers": {"draw": {"status": "connected"}},
    }
    failed = {
        "type": "system",
        "subtype": "init",
        "mcp_servers": {"draw": {"status": "failed"}},
    }
    assert (
        derive_mcp_summary([connected], "draw")["initialization_state"] == "connected"
    )
    assert derive_mcp_summary([failed], "draw")["initialization_state"] == "failed"
    list_connected = {
        "type": "system",
        "subtype": "init",
        "mcp_servers": [{"name": "draw", "status": "connected"}],
    }
    list_failed = {
        "type": "system",
        "subtype": "init",
        "mcp_servers": [{"name": "draw", "status": "failed"}],
    }
    assert (
        derive_mcp_summary([list_connected], "draw")["initialization_state"]
        == "connected"
    )
...
def test_opencode_tool_part_is_stable_and_correlated():
    ...
    assert (
        summary["selected_server_call_names"] == ["draw_create"]
