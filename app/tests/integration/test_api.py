...
def test_profile_revision_and_run_api():
    settings = Settings(
        ...
        claude_model_ids=["test-model"],
        claude_executable="/definitely/missing/claude",
    )
    client = TestClient(create_app(settings))
    profile = client.post(
        "/api/v1/profiles",
        json={"name": "demo", "mcp_json": {"mcpServers": {"srv": {"command": "echo"}}}},
    ).json()
    assert profile["revisions"][0]["revision_number"] == 1
    revision = profile["current_revision_id"]
    updated = client.post(
        f"/api/v1/profiles/{profile['id']}/revisions",
        json={"mcp_json": {"mcpServers": {"srv": {"command": "printf"}}}},
    )
    assert (
        updated.status_code == 201 and updated.json()["current_revision_id"] != revision
    )
    run = client.post(
        "/api/v1/runs",
        json={
