...
from mcp_pal.harness.acp import AcpHarnessRunner, _Client, protocol_probe
...
@pytest.mark.parametrize(
    "phase", ["initialize", "session/new", "mode", "config", "prompt"]
)
def test_one_global_deadline_covers_every_lifecycle_phase(tmp_path, phase):
    ...
    # Leave enough headroom for the subprocess to complete all phases before
    # the intentionally hanging one, even when the full process-heavy suite is
    # running.  The previous 200 ms budget made the asserted phase depend on
    # host scheduling rather than the lifecycle behavior under test.
    run = spec(
        agent,
        agent_mode_id="default" if phase == "mode" else None,
        session_config={"known": True} if phase == "config" else {},
        timeout_seconds=1,
    )
    result = asyncio.run(AcpHarnessRunner().run(run))
    assert result.status == "timed_out"
    assert result.error_code == "timed_out"
    expected_phase = {"mode": "set_session_mode", "config": "set_config_option"}.get(
        phase, phase
    )
...
def test_session_env_literals_and_refs_are_delivered_but_redacted(
...
for line in sys.stdin:
 ...
 if m=="initialize": print(json.dumps({{"jsonrpc":"2.0","id":r["id"],"result":{{"protocolVersion":1}}}}),flush=True)
  ...
  server=r["params"]["mcpServers"][0]
  handoff=server["args"][server["args"].index("--env-file")+1]
  open({str(marker)!r},"w").write(json.dumps({{"server_env":server["env"],"handoff":json.load(open(handoff)),"handoff_path":handoff}}))
...
""",
    ...
    run = AcpRunSpec(
        ...
        "agent-default",
        {
            "mcpServers": {
                "x": {
                    "command": "echo",
                    "env": {"LITERAL": "literal-secret", "REF": "${ACP_REF_SECRET}"},
                }
            }
        },
        "x",
    ...
    assert delivered["server_env"] == []
    assert delivered["handoff"]["environment"]["LITERAL"] == "literal-secret"
    assert delivered["handoff"]["environment"]["REF"] == "resolved-ref-secret"
    assert not Path(delivered["handoff_path"]).exists()
    evidence = json.dumps(result.event_records)
    assert "literal-secret" not in evidence and "resolved-ref-secret" not in evidence
    assert all(
        not frame.get("payload", {})
        .get("params", {})
        .get("mcpServers", [{}])[0]
        .get("env", [])
        for frame in result.event_records
    )
    late_secrets: set[str] = set()
    callback_values: list[object] = []
    client = _Client(
        [],
        [],
        callback=lambda value, *_args: callback_values.append(value),
        secrets=late_secrets,
    )
    late_secrets.add("late-acp-secret")
    asyncio.run(client.session_update("session", {"text": "late-acp-secret"}))
    assert callback_values == [{"text": "[REDACTED]"}]


def test_protocol_events_redact_selected_env_secrets_after_wire_capture(
    tmp_path, monkeypatch
):
    monkeypatch.setenv("WIRE_REF_SECRET", "wire-reference-secret")
    server = executable(
        tmp_path / "secret_mcp.py",
        """
import json,os,sys
for line in sys.stdin:
 r=json.loads(line);m=r.get("method")
        ...
        "agent-default",
        {
            "mcpServers": {
                "x": {
                    "command": server,
                    "env": {
                        "WIRE_LITERAL": "literal-wire-secret",
                        "WIRE_REF": "${WIRE_REF_SECRET}",
                    },
                }
            }
        },
...
def test_missing_selected_reference_preflight_before_process(
    tmp_path, monkeypatch, field
):
    marker = tmp_path / "started"
    agent = executable(
        tmp_path / "agent.py", f'open({str(marker)!r},"w").write("started")'
    )
    server = (
        {"command": "echo"}
        if field == "env"
            ...
            "type": "http",
            "url": "http://example.test/${MISSING_REF}",
            "headers": {"Authorization": "${MISSING_REF}"},
        }
    )
    if field == "env":
        server = {"command": "echo", "env": {"TOKEN": "${MISSING_REF}"}}
    ...
    monkeypatch.setattr(
        runtime.tempfile,
        "mkdtemp",
        lambda **kwargs: called.append(True) or str(tmp_path / "workspace"),
    )
    run = AcpRunSpec(
        "p", "agent-default", {"mcpServers": {"x": server}}, "x", {"command": agent}
    )
    result = asyncio.run(AcpHarnessRunner().run(run))
    assert (
        result.error_code == "acp_environment_missing"
        and result.error_phase == "preflight"
    )
...
@pytest.mark.parametrize(
    "body,code",
    [
        ("print('ordinary diagnostic not-json',flush=True)", "acp_malformed_stdout"),
        ("import sys;sys.exit(0)", "acp_early_exit"),
        ("import sys;sys.exit(7)", "acp_early_exit"),
    ],
)
def test_malformed_and_clean_or_nonzero_early_exit_are_structured(tmp_path, body, code):
    result = asyncio.run(
        AcpHarnessRunner().run(
            spec(executable(tmp_path / "bad.py", body), timeout_seconds=2)
        )
    )
...
def test_stderr_and_malformed_excerpt_redact_known_values_keep_diagnostics(
    tmp_path, monkeypatch
):
...
""",
    ...
    run = AcpRunSpec(
        ...
        {
            "mcpServers": {
                "x": {"command": "echo", "env": {"L": "literal-resolved-secret"}}
            }
        },
        "x",
        {"command": agent, "env": {"S": "${HARNESS_SECRET}"}},
        timeout_seconds=2,
    )
    result = asyncio.run(AcpHarnessRunner().run(run))
...
""",
    ...
    probe = asyncio.run(
        protocol_probe({"command": probe_agent, "env": {"S": "${HARNESS_SECRET}"}})
    )
...
""",
    ...
    assert (
        asyncio.run(AcpHarnessRunner().run(spec(optional, timeout_seconds=2))).status
        == "completed"
    )
    auth = executable(
        tmp_path / "auth.py",
...
@pytest.mark.parametrize(
    "method,code",
    [
        ("request_permission", "acp_interaction_required"),
        ("create_elicitation", "acp_interaction_required"),
        ("read_text_file", "acp_interaction_required"),
        ("create_terminal", "acp_interaction_required"),
    ],
)
def test_interaction_requests_are_structured(tmp_path, method, code):
    agent = executable(
        tmp_path / f"{method}.py",
        f'''
import json,sys
pending=None
for line in sys.stdin:
...
def test_stale_mode_and_config_options_are_rejected(tmp_path):
    ...
    stale_mode = asyncio.run(
        AcpHarnessRunner().run(spec(agent, agent_mode_id="missing", timeout_seconds=2))
    )
    assert (
        stale_mode.error_code == "acp_stale_option"
        and stale_mode.error_phase == "set_session_mode"
    )
    stale_config = asyncio.run(
        AcpHarnessRunner().run(
            spec(agent, session_config={"missing": True}, timeout_seconds=2)
        )
    )
    assert (
        stale_config.error_code == "acp_stale_option"
        and stale_config.error_phase == "set_config_option"
    )
...
""",
    ...
    mode = asyncio.run(
        AcpHarnessRunner().run(spec(agent, agent_mode_id="old", timeout_seconds=2))
    )
    config = asyncio.run(
        AcpHarnessRunner().run(
            spec(agent, session_config={"old": True}, timeout_seconds=2)
        )
    )
...
def test_updates_final_text_and_errors_redact_exact_manifest_secret(
    tmp_path, monkeypatch
):
...
""",
    ...
    async def capture(raw, kind, payload):
        seen.append((raw, payload))
...
def test_probe_metadata_and_protocol_errors_redact_exact_manifest_secret(
    tmp_path, monkeypatch
):
    monkeypatch.setenv("ORDINARY_SETTING", "probe-private-value")
    metadata_agent = executable(
        tmp_path / "probe-secret.py",
        """
import json,os,sys
for line in sys.stdin:
 r=json.loads(line); m=r.get("method")
...
""",
    ...
    manifest = {
        "command": metadata_agent,
        "env": {"CHILD_VALUE": "${ORDINARY_SETTING}"},
    }
    ...
    error_agent = executable(
        tmp_path / "error-secret.py",
        """
import json,os,sys
for line in sys.stdin:
 r=json.loads(line)
    ...
    failed = asyncio.run(
        AcpHarnessRunner().run(
            AcpRunSpec(
                ...
                {"mcpServers": {"x": {"command": "echo"}}},
                "x",
                {"command": error_agent, "env": {"CHILD_VALUE": "${ORDINARY_SETTING}"}},
                timeout_seconds=2,
            )
        )
    )
...
def test_parent_early_exit_reaps_child_and_does_not_wait_forever_on_inherited_stderr(
    tmp_path,
):
    child_pid = tmp_path / "early-child.pid"
    agent = executable(
        tmp_path / "early-parent.py",
        f"""
