...
def test_commands_and_exact_prompt_all_modes(tmp_path):
    ...
    for mode in ("mcp_only", "mcp_read_only", "full"):
        ...
        assert (
            "--mcp-config" in cmd
            and "--model" in cmd
            and "--expected-output" not in cmd
        )
        if mode == "mcp_only":
        ...
        if mode == "full":
            assert (
                "--tools" in cmd
                and cmd[cmd.index("--tools") + 1] == "default"
...
""",
    ...
    assert (
        result.cost_usd == 0.13
        and result.turns == 2
        and result.session_id == "session-1"
    )
    assert isinstance(result.events[3], str) and any(
        t == "thinking" for t, _ in result.normalized
    )
    assert derive_mcp_assertion(result.events, "draw") == "passed"
    summary = derive_mcp_summary(result.events, "draw")
    assert summary["selected_server_call_count"] == 1 and summary["success_count"] == 1


def test_opencode_config_command_and_native_events(tmp_path, monkeypatch):
    monkeypatch.setenv("DRAW_TOKEN", "draw-token-for-test")
    profile = {
        "mcpServers": {
            "draw": {
                "command": "node",
...
""",
    ...
    result = asyncio.run(
        OpenCodeRunner(script).run(
            RunSpec("exact prompt\n", "opencode/model", profile, "draw")
        )
    )
    assert (
        result.status == "completed"
        and result.final_text == "done"
        and result.session_id == "ses-1"
    )
    assert result.cost_usd == 0.02 and result.turns == 1
    assert derive_mcp_assertion(result.events, "draw") == "passed"
    assert {event for event, _ in result.normalized} >= {
        "step_start",
        "thinking",
        "tool_call",
        "tool_result",
...
""",
    ...
    profile = {
        "mcpServers": {"draw": {"command": sys.executable, "args": [str(fixture)]}}
    }
    result = asyncio.run(
        OpenCodeRunner(script).run(
            RunSpec("prompt", "model", profile, "draw", timeout_seconds=3)
        )
    )
    assert result.status == "completed"
    assert result.transport == "stdio"
    methods = [
        event.get("payload", {}).get("method") for event in result.protocol_events
    ]
    assert "tools/call" in methods
    request = next(
        event
        for event in result.protocol_events
        if event.get("payload", {}).get("method") == "tools/call"
    )
    response = next(
        event
        for event in result.protocol_events
        if event.get("payload", {}).get("id") == request["payload"]["id"]
    ...
    assert (
        isinstance(call["server_latency_ms"], (int, float))
        and call["server_latency_ms"] >= 0
    )
    assert (
        call["provenance"]["wire"] is True
...
def test_opencode_embedded_env_refs_and_hostile_home_isolation(tmp_path):
    ...
    assert (
        config["mcp"]["draw"]["headers"]["Authorization"] == "Bearer {env:DRAW_TOKEN}"
    )
    hostile = tmp_path / "home"
    (hostile / ".config/opencode").mkdir(parents=True)
    (hostile / ".config/opencode/opencode.json").write_text(
        '{"permission":{"*":"allow"}}'
    )
    marker = tmp_path / "env.json"
    profile = {"mcpServers": {"draw": {"command": "echo"}}}
    script = fake(
        tmp_path / "opencode-isolated.py",
        f"""
import json,os
...
""",
    ...
    try:
        result = asyncio.run(
            OpenCodeRunner(script, "secret-not-printed").run(
                RunSpec("prompt", "open/model", profile, "draw")
            )
        )
    finally:
        if old is None:
...
""",
    ...
    class Proxy:
        ...
        async def start(self):
            return "http://127.0.0.1:9191/mcp"
    ...
    result = asyncio.run(
        OpenCodeRunner(script).run(
            RunSpec("prompt", "model", profile, "draw", timeout_seconds=2)
        )
    )
    ...
    assert (
        Proxy.instances[0].kwargs["configured_headers"]["Authorization"]
        == "Bearer TOP-SECRET"
    )
...
def test_opencode_remote_proxy_failure_is_stable_and_stops(tmp_path, monkeypatch):
    class Proxy:
        ...
        async def start(self):
            raise RuntimeError("upstream secret https://upstream.example")
    ...
    result = asyncio.run(
        OpenCodeRunner("missing-opencode").run(
            RunSpec("prompt", "model", profile, "draw", timeout_seconds=1)
        )
    )
    assert (
        result.status == "failed" and result.error == "OpenCode transport setup failed"
    )
    assert (
        Proxy.instance.stopped
        and "SECRET" not in (result.error or "")
        and "upstream" not in (result.error or "")
    )
...
def test_opencode_proxy_is_stopped_when_config_write_fails(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(opencode_module, "McpHttpProxy", Proxy)
    monkeypatch.setattr(
        opencode_module.Path,
        "write_text",
        lambda *args, **kwargs: (_ for _ in ()).throw(OSError("nope")),
    )
    profile = {
        "mcpServers": {"draw": {"type": "http", "url": "https://upstream.example/mcp"}}
    }
    result = asyncio.run(
        OpenCodeRunner("missing-opencode").run(
            RunSpec("prompt", "model", profile, "draw", timeout_seconds=1)
    ...
    assert (
        ...
        and result.error == "OpenCode run setup failed"
        and Proxy.instance.stopped
    )


def test_opencode_clean_exit_recovers_export_before_classification(tmp_path):
    script = fake(
        tmp_path / "opencode-early.py",
        """
import json,sys
...
def test_opencode_recovers_after_intermediate_tool_step(tmp_path):
    script = fake(
        tmp_path / "opencode-tool-early.py",
        """
import json,sys
if 'export' in sys.argv:
...
def test_opencode_clean_exit_without_complete_trace_fails(tmp_path):
    script = fake(
        tmp_path / "opencode-incomplete.py",
        """
import json,sys
if 'session' in sys.argv: raise SystemExit(0)
...
def test_fake_opencode_api_persists_normalized_trace_and_report(tmp_path):
    ...
    class FakeRunner:
        async def run(self, spec, on_event=None, cancel_event=None):
            ...
            protocol = [
                {
                    ...
                    "payload": {
                        ...
                        "params": {
                            "name": "echo",
                            "arguments": {"text": "hello", "api_key": "SUPER-SECRET"},
                        },
                    },
                },
                {
            ...
            if on_event:
                for kind, payload in normalize_events(raw, spec.enabled_server):
                    await on_event(raw, kind, payload)
            return HarnessResult(
                status="completed",
                events=[raw],
                event_records=[{"raw_event": raw, "offset_ms": 4, "type": "tool_use"}],
                final_text="done",
    ...
    with TestClient(app) as client:
        ...
        assert (
            ...
            and trace["available"]
            and trace["schema"] == "opencode.v2"
        )
        assert (
            trace["mcp_calls_schema"] == "mcp.v1"
            and trace["summary"]["transport"] == "stdio"
        )
        call = trace["mcp_calls"][0]
        assert {call[k] for k in ("server", "tool", "status")} == {
            "draw",
            "echo",
            "completed",
        }
        assert call["arguments"] == {"text": "hello", "api_key": "[REDACTED]"} and call[
            "result"
        ] == {"text": "hello"}
        assert call["wire_request"]["method"] == "tools/call"
        ...
        wire_span = next(
            ...
            if span["kind"] == "mcp" and span["name"] == "tools/call"
        )
...
""",
    ...
    assert result.status == "completed" and json.loads(marker.read_text()) == {
        "openrouter": True,
        "anthropic": True,
        "opencode_go": True,
    }
...
def test_opencode_saved_auth_is_copied_into_isolated_data_store(tmp_path):
    ...
    auth = original / "opencode/auth.json"
    auth.parent.mkdir(parents=True)
    auth.write_text("credential")
    marker = tmp_path / "auth-presence"
    script = fake(
        tmp_path / "opencode-saved-auth.py",
        f"""
import json,os,stat
...
""",
    ...
    os.environ["XDG_DATA_HOME"] = str(original)
    try:
        result = asyncio.run(OpenCodeRunner(script).run(spec(script, timeout=2)))
    finally:
        if old is None:
...
def test_opencode_harness_limits_are_not_reported_as_enforced(tmp_path):
    settings = Settings(
        database_path=str(tmp_path / "limits.db"),
        opencode_api_key="key",
        opencode_executable="opencode",
        opencode_model_ids=["open/model"],
        claude_model_ids=["claude/model"],
    )
    app = create_app(settings)
...
""",
    ...
    settings = Settings(
        database_path=str(tmp_path / "go-auth.db"),
        opencode_api_key=None,
        opencode_executable=script,
...
def test_hanging_opencode_export_is_killed_and_reaped(tmp_path):
    ...
    script = fake(
        tmp_path / "opencode-export-hang.py",
        f"""
import os,sys,time
if 'export' in sys.argv:
...
""",
    ...
    async def run_export():
        ...
        value = await runner._export_completed_session(
            "ses-hang", os.environ.copy(), str(tmp_path)
        )
    ...
    try:
        ...
        raise AssertionError("timed-out export subprocess is still alive")


def test_api_dispatches_opencode_and_validates_models(tmp_path):
    script = fake(
        tmp_path / "opencode-api.py",
        """
import json,sys
if '--help' in sys.argv: print('run --format --model --thinking --pure'); raise SystemExit(0)
...
""",
    ...
    settings = Settings(
        database_path=str(tmp_path / "open.db"),
    ...
    assert {x["harness"] for x in capabilities["harnesses"]} == {
        "claude-code",
        "opencode",
    }
    ...
    assert (
        ...
        and run["harness"] == "opencode"
        and run["claude_result"] == "open response"
    )
    assert (
        client.post("/api/v1/runs", json={**body, "model": "claude-model"}).status_code
        == 422
    )
...
""",
    ...
    started = time.monotonic()
    result = asyncio.run(ClaudeCodeRunner(script).run(spec(script, timeout=0.1)))
    ...
    async def cancelled():
        ...
        task = asyncio.create_task(
            ClaudeCodeRunner(script).run(spec(script, timeout=10), cancel_event=event)
        )
...
""",
    ...
    settings = Settings(
        database_path=str(tmp_path / "trace.sqlite"),
        anthropic_api_key="key",
        claude_executable=script,
        claude_model_ids=["m"],
    )
    ...
    profile = client.post(
        ...
        json={
            "name": "trace",
            "mcp_json": {"mcpServers": {"draw": {"command": "ignored"}}},
        },
    ).json()
    created = client.post(
        "/api/v1/runs",
    ...
    assert (
        len([span for span in report["trace"]["spans"] if span["kind"] == "model_turn"])
        == 1
    )
...
def test_readiness_and_active_delete_conflict(tmp_path):
    help_script = fake(
        tmp_path / "help.py",
        "import sys; print('--print --bare --output-format --verbose --strict-mcp-config --mcp-config --no-session-persistence')",
    )
    settings = Settings(
        database_path=str(tmp_path / "db.sqlite"),
        anthropic_api_key="key",
        claude_executable=help_script,
        claude_model_ids=["m"],
    )
    app = create_app(settings)
    client = TestClient(app)
...
def test_pagination_filters_and_clone_validation(tmp_path):
    ...
    assert (
        len(
            client.get(
                "/api/v1/runs",
                params={"limit": 2, "offset": 1, "profile_id": p["id"], "model": "m"},
            ).json()
        )
        == 2
    )
    ...
    assert (
        client.post(
            f"/api/v1/runs/{old['id']}/clone", json={"model": "not-configured"}
        ).status_code
        == 422
    )
    assert (
        client.post(
            f"/api/v1/runs/{old['id']}/clone", json={"prompt": "   "}
        ).status_code
        == 422
    )
