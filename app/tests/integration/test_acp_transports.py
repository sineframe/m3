...
class LocalMcpUpstream:
    ...
    async def _handle_sse(
        self, method: str, path: str, payload: dict | None, writer: asyncio.StreamWriter
    ):
        if method == "GET":
...
async def _run_real_transport(tmp_path: Path, transport: str):
    ...
    agent = executable(
        tmp_path / f"{transport}-agent.py", _agent_body(record_path, transport)
    )
    spec = AcpRunSpec(
        "call echo",
        "agent-default",
...
def _assert_real_round_trip(result, injected, upstream, transport, upstream_url):
    ...
    assert (
        result.transport
        == result.configured_transport
        == result.instrumented_transport
        == transport
    )
    assert injected["type"] == transport
    assert (
        injected["url"].startswith("http://127.0.0.1:")
        and injected["url"] != upstream_url
    )
    ...
    assert all(
        "upstream-secret" not in json.dumps(frame) for frame in result.event_records
    )
    payloads = [
        frame.get("payload")
        for frame in result.protocol_events
        if isinstance(frame.get("payload"), dict)
    ]
    assert [payload.get("method") for payload in payloads if payload.get("method")] == [
        "initialize",
        "tools/list",
        "tools/call",
    ]
    call = next(
        payload for payload in payloads if payload.get("method") == "tools/call"
    )
    assert call["params"]["arguments"] == {"text": "nonce-http-sse-42"}
    response = next(
        payload
        for payload in payloads
        if payload.get("id") == call["id"] and payload.get("result")
    )
    assert response["result"]["content"][0]["text"] == "nonce-http-sse-42"
    request_frame = next(
        frame for frame in result.protocol_events if frame.get("payload") == call
    )
    response_frame = next(
        frame for frame in result.protocol_events if frame.get("payload") == response
    )
    ...
    assert any(
        req["payload"].get("method") == "tools/call"
        for req in upstream.requests
        if req.get("payload")
    )
    updates = [
        frame.get("payload", {}).get("params", {}).get("update", {})
        for frame in result.event_records
        if frame.get("payload", {}).get("method") == "session/update"
    ]
...
def test_http_real_acp_mcp_round_trip_through_local_proxy(tmp_path):
    result, injected, upstream, url, runner, workspaces = asyncio.run(
        _run_real_transport(tmp_path, "http")
    )
...
def test_legacy_sse_real_acp_mcp_round_trip_through_local_proxy(tmp_path):
    result, injected, upstream, url, runner, workspaces = asyncio.run(
        _run_real_transport(tmp_path, "sse")
    )
...
""",
    ...
    spec = AcpRunSpec(
        "p",
        "agent-default",
        {"mcpServers": {"x": {"type": "http", "url": "http://127.0.0.1:9/mcp"}}},
        "x",
        {"command": agent},
        allow_private_upstream=True,
    )
    ...
    assert (
        ...
        and result.error == "acp_transport_not_advertised: http"
    )
...
""",
    ...
    assert result.event_records
    assert any(
        frame.get("payload", {}).get("method") == "session/cancel"
        for frame in result.event_records
    )
