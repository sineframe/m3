...
def test_protocol_probe_is_async_and_persists_latest_status(tmp_path):
    ...
    p = client.post(
        ...
        json={
            "name": "probe",
            "manifest": {"command": "definitely-missing"},
            "trusted_unsandboxed": True,
        },
    ).json()
    started = client.post(
        f"/api/v1/harness-profiles/{p['id']}/probe", params={"kind": "protocol"}
    )
    ...
    for _ in range(50):
        ...
        time.sleep(0.01)
    assert (
        probes[0]["status"] == "failed"
        and probes[0]["evidence"]["local_ready"] is False
    )
...
def test_api_full_probe_requires_verified_protocol(tmp_path):
    ...
    profile = client.post(
        ...
        json={
            "name": "no-protocol",
            "manifest": {"command": "missing-agent"},
            "trusted_unsandboxed": True,
        },
    ).json()
    full = client.post(
        f"/api/v1/harness-profiles/{profile['id']}/probe", params={"kind": "full"}
    )
    ...
    db.add(
        HarnessProbe(
            revision_id=profile["current_revision_id"],
            kind="protocol",
            status="failed",
            evidence={},
            agent_identity=None,
...
def test_capabilities_readiness_and_probe_dimensions_are_isolated(tmp_path):
    ...
    descriptor = next(x for x in caps["harnesses"] if x.get("profile_id") == p["id"])
    ...
    db.add(
        HarnessProbe(
            ...
            evidence={
                "modes": {
                    "current_mode_id": "mode-a",
                    "available_modes": [{"id": "mode-a"}],
                },
                "config_options": [{"id": "a", "type": "boolean"}],
            },
            agent_identity=None,
        )
    )
    ...
    db.close()
    first = client.post(
        f"/api/v1/harness-profiles/{p['id']}/probe",
        params={
            "kind": "full",
            "transport": "stdio",
            "mode_id": "mode-a",
            "session_config": '{"a":true}',
        },
    )
    assert first.status_code == 202
    probes = client.get(f"/api/v1/harness-profiles/{p['id']}/probes").json()
    assert probes[0]["mode_id"] == "mode-a" and probes[0]["session_config"] == {
        "a": True
    }
...
def test_probe_latest_dimension_and_identity_downgrade(tmp_path):
    ...
    db.add_all(
        [
            HarnessProbe(
                ...
                kind="protocol",
                status="verified",
                evidence={"modes": ["m1"], "config_options": ["x"]},
    ...
    descriptor = next(
        x
        for x in client.get("/api/v1/capabilities").json()["harnesses"]
        if x.get("profile_id") == p["id"]
    )
    assert (
        descriptor["protocol_verified"]
        and descriptor["full_verified"]
        and descriptor["agent_modes"] == ["m1"]
        and not any("Identity changed" in w for w in descriptor["warnings"])
    )
...
def test_latest_failed_protocol_hides_options(tmp_path):
    ...
    assert not d["protocol_verified"] and d["agent_modes"] == []
...
def test_identity_mismatch_downgrades_full_and_missing_identity_warns(tmp_path):
    ...
    assert not d["full_verified"] and any(
    ...
    assert d["full_verified"] and any(
...
def test_stable_config_order_is_one_dimension_and_failed_latest_invalidates(tmp_path):
    ...
    assert not d["full_verified"]


@pytest.mark.parametrize("transport", ["stdio", "http", "sse"])
def test_full_probe_real_transport_requires_exact_nonce_update_and_completion(
    tmp_path, transport
):
    from mcp_pal.harness.acp import full_probe
    ...
    evidence = asyncio.run(
        full_probe(
            {"command": _probe_agent(tmp_path / f"agent-{transport}.py")},
            mode_id="mode-a",
    ...
    assert (
        evidence["prompt_completed"]
        and evidence["tool_update"]
        and evidence["message_update"]
    )
...
def test_full_probe_accepts_optional_identity_and_streamed_message_chunks(
    tmp_path, behavior
):
    from mcp_pal.harness.acp import full_probe
    ...
    evidence = asyncio.run(
        full_probe(
            {"command": _probe_agent(tmp_path / f"agent-{behavior}.py", behavior)},
            transport="stdio",
        )
    )
    assert evidence["status"] == "verified", evidence
...
@pytest.mark.parametrize(
    "behavior", ["wrong_args", "wrong_result", "no_update", "wrong_stop"]
)
...
def test_api_full_probe_runs_real_local_transport_and_cleans_service(
    tmp_path, transport
):
    client = TestClient(
        create_app(Settings(database_path=str(tmp_path / f"{transport}.sqlite")))
    )
    profile = client.post(
        "/api/v1/harness-profiles",
        json={
            "name": transport,
            "manifest": {"command": _probe_agent(tmp_path / f"api-{transport}.py")},
            "trusted_unsandboxed": True,
        },
    ).json()
    protocol_job = client.post(
        f"/api/v1/harness-profiles/{profile['id']}/probe", params={"kind": "protocol"}
    )
    assert protocol_job.status_code == 202
    for _ in range(200):
        protocol_rows = client.get(
            f"/api/v1/harness-profiles/{profile['id']}/probes"
        ).json()
        if protocol_rows and protocol_rows[0]["status"] not in {"queued", "running"}:
            break
        time.sleep(0.02)
    ...
    descriptor = next(
        x
        for x in client.get("/api/v1/capabilities").json()["harnesses"]
        if x.get("profile_id") == profile["id"]
    )
    assert (
        descriptor["agent_modes"][0]["name"] == "Mode A"
        and descriptor["current_agent_mode_id"] == "default"
    )
    started = client.post(
        f"/api/v1/harness-profiles/{profile['id']}/probe",
        params={
            "kind": "full",
            "transport": transport,
            "mode_id": "mode-a",
