...
""",
    ...
    spec = AcpRunSpec(
        "call echo",
        "agent-default",
        {"mcpServers": {"echo": {"command": sys.executable, "args": [str(echo)]}}},
        "echo",
        {
    ...
    async def callback(raw, kind, payload):
        callbacks.append((kind, payload))

    result = asyncio.run(AcpHarnessRunner().run(spec, on_event=callback))
    assert result.status == "completed", result.error
    assert result.final_text == "nonce-123"
    assert any(
        kind == "acp_update" and "thought" in str(payload).lower()
        for kind, payload in callbacks
    )
    assert any(
        kind == "acp_update" and "tool_call" in str(payload).lower()
        for kind, payload in callbacks
    )
    assert any(
        x.get("payload", {}).get("method") == "tools/call"
        for x in result.protocol_events
    )
    assert all(
        "nonce-123" in json.dumps(x)
        for x in result.protocol_events
        if x.get("payload", {}).get("method") == "tools/call"
    )
    call = next(
        x
        for x in result.protocol_events
    ...
    response = next(
        x
        for x in result.protocol_events
        if x.get("payload", {}).get("id") == call["payload"]["id"]
        and x.get("direction") == "server_to_client"
    )
...
""",
    ...
    try:
        ...
        raise AssertionError("timed out ACP process still alive")
    runner = AcpHarnessRunner()
    pid_file.unlink()

    async def cancel():
        cancel_spec = AcpRunSpec(**{**spec.__dict__, "timeout_seconds": 2})
        task = asyncio.create_task(runner.run(cancel_spec))
...
def test_acp_missing_mcp_environment_fails_before_workspace(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        module.tempfile,
        "mkdtemp",
        lambda **kwargs: called.append(1) or str(tmp_path / "leak"),
    )
    spec = AcpRunSpec(
        "p",
        "agent-default",
        {
            "mcpServers": {
                "echo": {"command": "echo", "env": {"TOKEN": "${MISSING_ACP_TOKEN}"}}
            }
        },
        "echo",
