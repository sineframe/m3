...
def test_acp_tool_lifecycle_and_message_plan_state_updates_are_normalized():
    acp = [
        _frame(1, "server_to_client", {"method": "session/update", "params": {"update": {"sessionUpdate": "plan", "entries": [{"status": "pending"}]}}}),
        _frame(2, "server_to_client", {"method": "session/update", "params": {"update": {"sessionUpdate": "tool_call", "toolCallId": "t1", "title": "echo", "rawInput": {"x": 1}, "status": "running"}}}),
        _frame(4, "server_to_client", {"method": "session/update", "params": {"update": {"sessionUpdate": "tool_call_update", "toolCallId": "t1", "rawOutput": {"ok": 1}, "status": "completed"}}}),
        _frame(5, "server_to_client", {"method": "session/update", "params": {"update": {"sessionUpdate": "current_mode_update", "currentModeId": "safe"}}}),
        _frame(6, "server_to_client", {"method": "session/update", "params": {"update": {"sessionUpdate": "agent_message_chunk", "content": {"type": "text", "text": "done"}}}}),
    ]
    trace = build_acp_trace(acp_frames=acp, selected_server="s", configured_transport="http", instrumented_transport="proxy")
...
def test_wire_ids_are_typed_and_repeated_ids_use_request_sequence():
    wire = [
        _frame(1, "client_to_server", {"id": 1, "method": "tools/call", "params": {"name": "echo", "arguments": {"n": 1}}}),
        _frame(2, "client_to_server", {"id": 1, "method": "tools/call", "params": {"name": "echo", "arguments": {"n": 2}}}),
        _frame(3, "server_to_client", {"id": 1, "result": {"n": 1}}),
        _frame(4, "client_to_server", {"id": "1", "method": "tools/call", "params": {"name": "echo", "arguments": {"n": "string"}}}),
        _frame(5, "server_to_client", {"id": 1, "error": {"code": 9, "message": "bad"}}),
        _frame(6, "server_to_client", {"id": "1", "result": {"n": "string"}}),
    ]
    calls = build_acp_trace(acp_frames=[], mcp_frames=wire, selected_server="s")["mcp_calls"]
    assert [call["request_sequence"] for call in calls] == [1, 2, 4]
    assert calls[0]["result"] == {"n": 1} and calls[0]["status"] == "completed"
    assert calls[1]["error"] == {"code": 9, "message": "bad"} and calls[1]["status"] == "error"
...
def test_wire_only_and_acp_only_calls_are_preserved_and_inference_is_unambiguous():
    acp = [_frame(10, "server_to_client", {"method": "session/update", "params": {"update": {"sessionUpdate": "tool_call", "toolCallId": "a", "title": "echo", "rawInput": {"x": 1}}}})]
    wire = [
        _frame(11, "client_to_server", {"id": 7, "method": "tools/call", "params": {"name": "echo", "arguments": {"x": 1}}}),
        _frame(12, "server_to_client", {"id": 7, "result": {"ok": True}}),
        _frame(50, "client_to_server", {"id": 8, "method": "tools/call", "params": {"name": "other", "arguments": {}}}),
    ]
    trace = build_acp_trace(acp_frames=acp, mcp_frames=wire, selected_server="s", status="timed_out")
    assert trace["capture_status"] == "partial"
    assert len(trace["mcp_calls"]) == 2
    linked = next(call for call in trace["mcp_calls"] if call["id"] == 7)
    assert linked["provenance"] == {"model": True, "wire": True, "correlation": "inferred"}
    wire_only = next(call for call in trace["mcp_calls"] if call["id"] == 8)
    assert wire_only["provenance"] == {"model": False, "wire": True, "correlation": "wire"}

    # Two ACP candidates at the same time make inference intentionally
    # ambiguous; neither is silently joined to the wire record.
    acp += [_frame(20, "server_to_client", {"method": "session/update", "params": {"update": {"sessionUpdate": "tool_call", "toolCallId": "b", "title": "echo"}}})]
    ambiguous = build_acp_trace(acp_frames=acp, mcp_frames=wire[:2], selected_server="s")
...
def test_inferred_link_ignores_initialize_and_tools_list_mcp_event_spans():
    acp = [_frame(10, "server_to_client", {"method": "session/update", "params": {"update": {"sessionUpdate": "tool_call", "toolCallId": "t", "title": "echo", "rawInput": {"x": 1}, "status": "running"}}})]
    wire = [
        ...
        _frame(11, "client_to_server", {"id": 3, "method": "tools/call", "params": {"name": "echo", "arguments": {"x": 1}}}),
    ...
    assert call["provenance"] == {"model": True, "wire": True, "correlation": "inferred"}
    assert any(span["kind"] == "mcp_event" and span["name"] == "initialize" for span in trace["spans"])


def test_malformed_and_unknown_frames_remain_separate_and_tolerated():
    trace = build_acp_trace(
        acp_frames=["not-json", _frame(2, "server_to_client", {"method": "vendor/unknown", "params": {"_meta": {"x": 1}}})],
        mcp_frames=["not-json"],
        selected_server="s",
    )
